...
public sealed class AppComposition
{
        ...
        string? usageSpoolPath = null)
    {
        ...
        // 使用日志上报器:磁盘队列(先落盘后上传,崩溃/离线退出不丢记录,下次启动续传)。
        // 单实例共用,登录时绑定账号归属,切换账号不会用错 token 上传上一个账号的记录。
        this.usageReporter = usageReporter ?? new UsageLogUploader(otaClient, usageSpoolPath ?? DefaultUsageSpoolPath());
        ...
        // ROOT 云端 OTA 提取:复用 otaClient(登录态/钉扎),OTA 链接只留服务内存。
        var rootCloudExtract = new RootOtaCloudExtractService(otaClient, backend);
        ...
        // 完整性遥测:租约冲突/失效等客户端可检测的完整性事件上报(对齐 Rust integrity_reporter)。
        Integrity = new IntegrityReporter(otaClient);
        Heartbeat = new HeartbeatService(
            otaClient,
            onForceExitAsync: ForceExitAsync,
            onUpdateRequiredAsync: ShowUpdateRequiredAsync,
            integrityReporter: Integrity,
            isOperationBusy: () => Coordinator.IsBusy);
    ...
    /// <summary>完整性遥测上报器:pin 校验失败 / 租约冲突等事件的 best-effort 上报。</summary>
    public IntegrityReporter Integrity { get; }
    ...
    /// <summary>
    /// 登录成功后启动在线会话:设 token、绑定登录时生成的会话 id、启动心跳与在线状态轮询。
    /// 新版登录契约:登录已携带 session_id 在服务端创建租约,心跳必须复用同一 id(不再本地另生成)。
    /// 心跳会在本实例剩余生命周期内持续;App 退出时由 <see cref="StopAsync"/> 收尾(goodbye)。
    /// 幂等:重复调用不再重新绑定会话 id(运行中的心跳仍用旧 id,避免公开 id 与真实会话脱钩)。
    /// </summary>
    public void StartSessionAsync(string token, string username, string sessionId)
    {
        ...
        sessionStarted = true;
        SessionId = sessionId;
        // 使用日志绑定本次登录的账号归属(账号做 SHA-256 不透明化后落盘):
        // 只有同账号的记录会被上传,避免上一个账号的残留队列用当前 token 传上去。
        usageReporter.PublishSession(username, sessionId);
        Heartbeat.Start(SessionId);
        Online.Start();
        usageReporter.Start();
    }

    /// <summary>使用日志磁盘队列的默认位置(与操作日志同目录,跨进程保留待上传记录)。</summary>
    private static string DefaultUsageSpoolPath() => Path.Combine(
    ...
    public static AppComposition CreateForTesting(IFastbootRsNativeApi nativeApi, IProcessRunner processRunner) =>
        new(
            ...
            usageSpoolPath: Path.Combine(Path.GetTempPath(), "VivoKsu.Tests", $"usage-{Guid.NewGuid():N}.json"));
    ...
    public async Task StopAsync()
    {
        ...
        // 退出前 flush 待上传的使用日志(best-effort,带 5s 预算;失败不影响退出)。
        // 先 Stop() 停止接收新记录,再 CloseSessionAsync():未传完的记录已落盘,下次启动续传。
        try
        {
            usageReporter.Stop();
            await usageReporter.CloseSessionAsync(TimeSpan.FromSeconds(5));
        }
        catch
        {
            // 离线/超时:队列已落盘,下次登录续传;不阻塞退出。
        }
    ...
    /// <summary>
    /// 服务端强制退出(心跳返回 force_exit):唯一的进程终止路径。
    /// 无条件立即执行——不等当前操作(刷写/下载中也不例外)、不弹任何提示、
    /// 不做 StopAsync 收尾(goodbye/flush/清理都不跑,进程即刻消失;
    /// 服务端按在线窗口 purge 兜底清理会话行)。
    /// </summary>
    private async Task ForceExitAsync(string reason)
    {
        // 回调正由心跳循环内调用:绝不能等待循环或做任何长收尾,否则强制退出失去「立即」语义。
        // 先落一条日志(唯一留痕,进程随即退出)。
        try
        {
            LogService.Write(OperationLogLevel.Warning, $"服务端强制退出: {reason}");
        }
        catch
        {
            // 日志失败不阻塞终止。
