...
public sealed class FirmwarePartitionExtractor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("不支持 Vivo gzip 固件格式,请选择 payload.bin 或固件 zip。");
        }

        if (kind != FirmwareFormatDetector.FirmwareKind.PayloadZip)
        {
            throw new InvalidDataException("无法识别的固件格式(支持 payload.bin / 含 payload.bin 的固件 zip / 直接镜像 zip)。");
        }
