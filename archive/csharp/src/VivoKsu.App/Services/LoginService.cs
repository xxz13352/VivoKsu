...
public sealed class LoginService : IDisposable
{
    ...
    public LoginService(string baseUrl = OtaApiClient.DefaultBaseUrl)
    {
        ...
        // 钉扎策略与 OtaApiClient 一致:登录请求同样不允许被 MITM/重定向劫持。
        http = new HttpClient(ApiTlsPinPolicy.CreateHandler()) { Timeout = TimeSpan.FromSeconds(30) };
    ...
    public async Task<LoginResult> LoginAsync(string username, string password, CancellationToken cancellationToken)
    {
        ...
        // 新版登录契约(2026-08-23 签名租约):登录必须携带完整绑定四元组,
        // 服务端据此创建 session_leases(首序号 1),缺失/非法返回 400「client_version 不合法」。
        // 会话 id 在登录时生成并随 LoginResult 返回,心跳/下线必须复用同一值。
        var sessionId = ClientSession.NewSessionId();
        ...
        return new LoginResult(token, username, name ?? username, sessionId);
    }
...
/// <summary>登录成功返回的结果。SessionId 为登录时生成、服务端租约绑定的会话 id。</summary>
public sealed record LoginResult(string Token, string Username, string Name, string SessionId);
