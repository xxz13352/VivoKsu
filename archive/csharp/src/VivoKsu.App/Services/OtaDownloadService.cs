...
public sealed class OtaDownloadService : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new IOException($"无法确定固件包大小(服务器未返回 Content-Length): {url}");
        }
        ...
        {
            throw new IOException($"下载固件包失败: {exception.Message}", exception);
        }
        finally
        {
            service.DownloadProgressChanged -= handler;
        ...
        {
            throw new IOException($"下载固件包失败: {completed.Error.Message}", completed.Error);
        }

        if (completed?.Cancelled == true)
        {
            throw new OperationCanceledException("下载固件包已取消");
        }
    }
