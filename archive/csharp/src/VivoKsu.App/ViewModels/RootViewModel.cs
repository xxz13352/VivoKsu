...
public partial class RootViewModel : ObservableObject
{
    ...
    private readonly RootOtaCloudExtractService? cloudExtract;
    ...
    /// <summary>云端提取状态文案(不含 URL:对齐 Rust「URL 不进浏览器」)。</summary>
    [ObservableProperty]
    private string cloudOtaSummary = "可从服务端自动提取启动镜像,无需手动下载";

    [ObservableProperty]
    private bool isCloudOtaBusy;
    ...
    public RootViewModel(
        ...
        RootOtaCloudExtractService? cloudExtract = null)
    {
        ...
        this.cloudExtract = cloudExtract;
        ...
        ExtractCloudOtaCommand = new AsyncRelayCommand(ExtractCloudOtaAsync, () => !IsCloudOtaBusy);
    ...
    /// <summary>云端 OTA 提取:读设备版本 → 解析 OTA → 按需流式提取启动镜像(不下载整包)。</summary>
    public IAsyncRelayCommand ExtractCloudOtaCommand { get; }
    ...
    /// <summary>
    /// 云端 OTA 提取入口:读设备 PD/版本 → 服务端解析 → 远程 ZIP 按需流式提取启动镜像。
    /// 提取产物直接灌入 SelectedImage/SelectedVendorImage,后续修补与刷写流程与手选镜像完全一致。
    /// OTA 链接只存在于服务层内存,日志与状态文案均不包含 URL。
    /// </summary>
    private async Task ExtractCloudOtaAsync()
    {
        if (cloudExtract is null || backend is null)
        {
            CloudOtaSummary = "云端提取不可用(缺少设备后端)。";
            return;
        }

        var serial = session.Details.Serial;
        if (string.IsNullOrWhiteSpace(serial) || serial == "--")
        {
            CloudOtaSummary = "未检测到 ADB 设备,云端提取需要设备在线读取版本。";
            logs.Write(OperationLogLevel.Warning, "云端提取:未检测到 ADB 设备。");
            return;
        }
        ...
        try
        {
            if (coordinator is not null)
            {
                await RunCoordinatedAsync(OperationKind.Transferring, "云端提取 ROOT 镜像", async (context, cancellationToken) =>
                {
                    await ExtractCloudOtaCoreAsync(context, cancellationToken);
                    context.ReportStage("云端提取完成");
                });
            }
            else
            {
                session.BeginOperation(OperationKind.Transferring, "云端提取 ROOT 镜像");
                try
                {
                    await ExtractCloudOtaCoreAsync(null, CancellationToken.None);
                    session.CompleteOperation("云端提取完成");
                    logs.Write(OperationLogLevel.Success, "云端提取完成。");
                }
                catch (Exception exception)
                {
                    session.FailOperation("云端提取失败");
                    logs.Write(OperationLogLevel.Error, $"云端提取失败: {exception.Message}");
                    CloudOtaSummary = $"云端提取失败: {exception.Message}";
                }
            }
        }
        finally
        {
            IsCloudOtaBusy = false;
        }
    }

    /// <summary>云端提取核心流程(在 coordinator 操作/本地会话内执行)。</summary>
    private async Task ExtractCloudOtaCoreAsync(OperationContext? context, CancellationToken cancellationToken)
    {
        var serial = session.Details.Serial;
        void Report(string stage)
        {
            context?.ReportStage(stage);
            CloudOtaSummary = stage;
        }

        Report("正在读取设备版本");
        var (version, codename) = await cloudExtract!.ReadDeviceVersionAsync(serial, cancellationToken);
        // 设备代号优先用版本串解析出的 codename,回退到 getprop 的 ro.product.device。
        var pd = !string.IsNullOrWhiteSpace(codename) ? codename : session.Details.Codename;
        if (string.IsNullOrWhiteSpace(pd) || string.IsNullOrWhiteSpace(version) || pd == "--")
        {
            CloudOtaSummary = "无法读取设备版本,请确认 ADB 已连接并授权后重试。";
            logs.Write(OperationLogLevel.Warning, $"云端提取:版本读取失败(pd={pd})。");
            throw new InvalidOperationException("无法读取设备版本号。");
        }

        Report($"设备 {pd} · 正在查询固件");
        logs.Write(OperationLogLevel.Info, $"云端提取:设备 {pd},版本 {version}。");

        var stagingRoot = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            "VivoKsu",
            "root-ota");
        var images = await cloudExtract.ExtractAsync(pd, version, stagingRoot, Report, cancellationToken);

        if (images.BootImage is not null)
        {
            SelectedImage = await imageInspector.InspectImageAsync(images.BootImage.Path, cancellationToken);
        }

        if (images.VendorBoot is not null)
        {
            SelectedVendorImage = await imageInspector.InspectImageAsync(images.VendorBoot.Path, cancellationToken);
        }

        var summary = $"云端提取完成: {images.BootPartitionName}"
            + (images.VendorBoot is not null ? " + vendor_boot" : string.Empty);
        logs.Write(OperationLogLevel.Success, $"{summary}(暂存于 {images.StagingDirectory})。");
        CloudOtaSummary = summary;
    }
