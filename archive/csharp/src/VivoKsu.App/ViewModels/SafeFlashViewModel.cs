...
/// <summary>
/// 安全刷写:查询设备(adb)→ 在线获取固件(或选择本地 .zip / payload.bin)→ 解压解包
/// → 按所选刷写选项执行 → 重启设备。
...
public partial class SafeFlashViewModel : ObservableObject
{
    ...
    /// <summary>① 下载+刷入:读设备版本 → 在线获取固件 → 下载 → 准备刷写。</summary>
    private async Task DownloadAndFlashAsync()
    {
        ...
        var downloadSuccess = await RunOperationAsync(OperationKind.Transferring, "下载固件包", async (context, ct) =>
        {
            context.ReportStage("正在请求线刷系统");
            otaClient.BaseUrl = OtaApiClient.DefaultBaseUrl;
            var rom = await otaClient.ResolveAsync(pd, version, ct);

            var fileName = $"{SanitizeFileNamePart(pd)}_{SanitizeFileNamePart(version)}_firmware.zip";
            var destination = Path.Combine(stagingRoot, fileName);
            context.ReportStage("正在多线程下载固件包");
            StartElapsedTicker();
            try
            {
    ...
    private async Task PrepareFlashAsync()
    {
        ...
        ConfirmSummary = $"将刷入 {FlashCount} 个分区,完成后自动重启设备。";
        if (!IsSafeFlash)
        {
        ...
        {
            var blockWarning = "⚠ 固件含暂不支持刷写的分区内容,这些分区将保持原样。";
