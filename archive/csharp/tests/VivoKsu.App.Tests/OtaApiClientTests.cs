...
public class OtaApiClientTests
{
    ...
    [Fact]
    public async Task HeartbeatAsync_posts_the_full_signed_session_binding()
    {
        ...
        var result = await client.HeartbeatAsync("sess-abc", sequence: 1, active: true, CancellationToken.None);

        handler.Path.Should().Be("/api/heartbeat");
        handler.Body.Should().Contain("\"session_id\":\"sess-abc\"")
            .And.Contain("\"active\":true")
            .And.Contain("\"client_version\"")
            .And.Contain("\"build_id\"")
            .And.Contain("\"process_nonce\"")
            .And.Contain("\"sequence\":1");
        result.ForceExit.Should().BeFalse();
        result.Reason.Should().BeNull();
    }

    [Fact]
    public async Task HeartbeatAsync_parses_lease_sequence_from_response_payload()
    {
        var leasePayload = Convert
            .ToBase64String(System.Text.Encoding.UTF8.GetBytes("""{"sequence":2}"""))
            .TrimEnd('=').Replace('+', '-').Replace('/', '_');
        var client = new OtaApiClient(
            new HttpClient(new BodyCapturingHandler((
                $$"""{"ok":true,"force_exit":false,"lease_payload":"{{leasePayload}}"}""",
                HttpStatusCode.OK))),
            baseUrl: "https://localhost:7243");
    ...
    [Fact]
    public async Task Goodbye_heartbeat_posts_only_session_id_and_active_flag()
    {
        var handler = new BodyCapturingHandler(("""{"ok":true,"force_exit":false}""", HttpStatusCode.OK));
        var client = new OtaApiClient(new HttpClient(handler), baseUrl: "https://localhost:7243");

        await client.HeartbeatAsync("sess-abc", sequence: 0, active: false, CancellationToken.None);

        handler.Body.Should().Contain("\"session_id\":\"sess-abc\"")
            .And.Contain("\"active\":false")
    ...
    public async Task HeartbeatAsync_maps_426_to_update_required()
    {
        ...
        var act = () => client.HeartbeatAsync("sess-abc", sequence: 1, active: true, CancellationToken.None);
    ...
    [Fact]
    public async Task ReportIntegrityAsync_posts_a_snake_case_body_to_the_integrity_endpoint()
    {
        var handler = new BodyCapturingHandler(("""{"ok":true}""", HttpStatusCode.OK));
        var client = new OtaApiClient(new HttpClient(handler), baseUrl: "https://localhost:7243");

        await client.ReportIntegrityAsync(new IntegrityReportRequest
        {
            EventId = "evt-1",
            Phase = IntegrityReportPhase.SessionRestore,
            Reason = IntegrityReportReason.LeaseSignatureInvalid,
            ClientVersion = AppInfo.Version,
            BuildId = ClientSession.BuildId,
            OccurredAtEpochSeconds = 1_765_000_000,
        }, CancellationToken.None);

        handler.Path.Should().Be("/api/integrity/report");
        handler.Body.Should().Contain("\"event_id\":\"evt-1\"")
            .And.Contain("\"phase\":\"session_restore\"")
            .And.Contain("\"reason\":\"lease_signature_invalid\"")
            .And.Contain("\"occurred_at\":1765000000")
            .And.Contain("\"build_id\"");
    }
    ...
    [InlineData("bad id!")]
    [InlineData("")]
    [InlineData("01234567890123456789012345678901234567890123456789012345678901234")] // 65 位,超 64 上限
    public async Task ReportIntegrityAsync_rejects_an_invalid_event_id_before_sending(string eventId)
    {
        var handler = new BodyCapturingHandler(("""{"ok":true}""", HttpStatusCode.OK));
        var client = new OtaApiClient(new HttpClient(handler), baseUrl: "https://localhost:7243");

        var act = () => client.ReportIntegrityAsync(new IntegrityReportRequest
        {
            EventId = eventId,
            Phase = IntegrityReportPhase.Startup,
        ...
        handler.Body.Should().BeNull(); // 非法载荷不得产生网络往返
    }

    [Fact]
    public async Task UploadCrashReportAsync_posts_the_crash_body_and_accepts_202()
    {
        var handler = new BodyCapturingHandler(("""{"ok":true}""", HttpStatusCode.Accepted));
        var client = new OtaApiClient(new HttpClient(handler), baseUrl: "https://localhost:7243");

        await client.UploadCrashReportAsync(new CrashReportRequest
        {
            EventId = "crash-1765000000-42",
            ClientVersion = AppInfo.Version,
            BuildId = ClientSession.BuildId,
            SessionId = ClientSession.NewSessionId(),
            PanicMessage = "panic: 索引超出范围。",
            Backtrace = "   at VivoKsu.App.Main",
            OccurredAtEpochSeconds = 1_765_000_000,
        }, CancellationToken.None);

        handler.Path.Should().Be("/api/diagnostics/crash");
        handler.Body.Should().Contain("\"event_id\":\"crash-1765000000-42\"")
            .And.Contain("\"panic_message\"")
            .And.Contain("\"backtrace\"")
            .And.Contain("\"occurred_at\":1765000000");
    }

    [Fact]
    public async Task UploadCrashReportAsync_treats_200_as_the_idempotent_duplicate_success()
    {
        var client = new OtaApiClient(
            new HttpClient(new BodyCapturingHandler(("""{"duplicate":true}""", HttpStatusCode.OK))),
            baseUrl: "https://localhost:7243");

        var act = () => client.UploadCrashReportAsync(new CrashReportRequest
        {
            EventId = "crash-dup",
            ClientVersion = AppInfo.Version,
            BuildId = ClientSession.BuildId,
    ...
    [Fact]
    public async Task UploadCrashReportAsync_rejects_an_oversized_panic_message_before_sending()
    {
        var handler = new BodyCapturingHandler(("""{"ok":true}""", HttpStatusCode.Accepted));
        var client = new OtaApiClient(new HttpClient(handler), baseUrl: "https://localhost:7243");
        ...
        var act = () => client.UploadCrashReportAsync(new CrashReportRequest
        {
            ...
            BuildId = ClientSession.BuildId,
            SessionId = ClientSession.NewSessionId(),
            PanicMessage = new string('x', CrashReportRequest.MaxPanicMessageBytes + 1),
            Backtrace = string.Empty,
            OccurredAtEpochSeconds = 1_765_000_000,
        }, CancellationToken.None);
    ...
    [Fact]
    public async Task UploadUsageLogsAsync_returns_the_server_received_count()
    {
        var client = new OtaApiClient(
            new HttpClient(new BodyCapturingHandler(("""{"ok":true,"received":3}""", HttpStatusCode.OK))),
            baseUrl: "https://localhost:7243");

        var result = await client.UploadUsageLogsAsync(
            [new UsageLogEntry("flash", "刷入", "success", "e1", 1L, null, null)],
            CancellationToken.None);
    ...
    [Fact]
    public async Task Conflict_is_mapped_to_a_relogin_message()
    {
        var client = new OtaApiClient(
            new HttpClient(new StubHandler(("""{}""", HttpStatusCode.Conflict))),
            baseUrl: "https://localhost:7243");

        var act = () => client.GetOnlineAsync(CancellationToken.None);

        (await act.Should().ThrowAsync<OtaApiException>())
            .Which.Message.Should().Contain("重新登录");
    }
