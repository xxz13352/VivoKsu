...
- **鉴权**: `/api/rom`、心跳、在线列表、操作授权、使用日志和 V2 trace 必须使用 `Authorization: Bearer <API token>`;完整性/崩溃遥测仍允许匿名（各端点规则见下文）
- **版本门禁**: 登录、会话恢复、心跳、操作授权、使用日志、V2 trace 和 ROM 查询必须带合法 `X-Nwflash-Version`;缺失/非法头拒绝，低于后台最低版本 → **426 强制更新**(见 [版本门禁](#版本门禁强制更新))。健康检查、pin、遥测、崩溃补传、版本发现和客户端在线列表是明确豁免端点
...
> **客户端义务**:`panic_message`/`backtrace` 在发送前完成**价值过滤**——已识别的凭据字段、URL userinfo、私钥/高风险内容会被替换或拒绝;路径、序列号、命令行等有诊断价值的内容**保留**。服务端只做结构校验,不做二次过滤。报告保留 90 天,由 Worker Cron 清理。
...
以下业务端点必须携带客户端版本号,服务端在密码、bearer、租约、日志、trace 或 VOTA 副作用之前校验:
...
`OPTIONS`、`GET /health`、`GET /api/security/pins`、`POST /api/integrity/report`、`POST /api/diagnostics/crash`、`GET /api/app/version` 和 API 客户端视角的 `GET /api/online` 不要求该头;这些豁免不改变各自的鉴权、签名、字段或限流规则。未知路径仍返回 `404`。
...
```json
{ "error": "缺少 X-Nwflash-Version。", "code": "CLIENT_VERSION_REQUIRED" }
```

存在但不符合 `A-Za-z0-9._+-`、首字符字母/数字、最多 32 字符的现有格式返回 `400`:

```json
{ "error": "X-Nwflash-Version 格式无效。", "code": "CLIENT_VERSION_INVALID" }
```

服务端不回显原始版本头。上述版本错误和 426 响应带 CORS、`Content-Type: application/json` 与 `Cache-Control: no-store`;缺/非法/低版本请求不会写入 D1 或调用上游。合法版本等于或高于最低版本继续原端点语义。
...
密码验证成功后,若 D1 中的 `api_users.token` 以 `revoked:` 开头,Worker 会先用单条条件 `UPDATE ... RETURNING` 将该精确 marker compare-and-swap 为新的 32-byte(64 位小写 hex)token。CAS 谓词同时绑定最初验证的 user ID、密码 hash、salt、旧 token、enabled 和 banned。并发登录的 CAS 败者只有在重新读取并确认密码 hash/salt 仍属于本次已验证代际、账号仍启用且胜者 token 非 revoked 后才能复用胜者 token;否则明确返回 `409`,不签 lease。revoked marker 永远不会作为 bearer、响应字段、日志或错误内容返回。
...
服务端在请求字段、账号、密码、封禁和停用检查通过并完成上述 token 交换后,会先用条件 D1 mutation 将登录线性化到最初验证的完整 `id + password hash + salt + 最终 token + enabled + banned` 代际;普通非 revoked token 登录也必须通过同一条件 mutation。只有 mutation 获胜才计算最终 token 的 `token_sha256` 和生成签名候选。写 `session_leases` 时再以同一凭据代际执行条件 `INSERT ... SELECT`,因此改密若发生在签名计算期间也不会留下或返回 stale session/lease。session ID 已存在或凭据代际已变化均返回 `409`,不会返回 token 或租约。缺失/畸形签名 key 返回 `503` 且不创建会话状态;签名服务故障期间既有 `400/401` 登录失败仍保持原语义。

**已签发 lease 的撤销边界**:改密会立即轮换数据库 token 并删除服务端 session,所以旧 bearer 后续认证/心跳无法续租;但客户端在改密提交前已经收到并验签的 lease 是一个离线可验证的密码学凭据,无法从服务端追溯抹除,最多持续到自身 `expires_at`(当前 TTL 120 秒)。客户端仍须按心跳/认证的 401 立即退出,不得把未过期的旧 lease 当作绕过在线撤销的依据。本次修复的线性化点只保证改密提交后继续运行的旧登录请求不会再签发或写入新的 lease/session。

**失败**:`401` —— `用户名或密码错误` / `账号已被封禁,请联系管理员。` / `账号已被停用。`;`400` 缺少或非法字段;`409` session ID 冲突、token 交换未决或凭据代际已变化;`503` 签名服务不可用。错误体不包含旧 token、新 token 或 revoked marker。
...
```
版本门禁先于 bearer 校验:低版本请求可先收到 `426`;通过版本门禁后,不带 bearer 返回 `200 { "loggedIn": false }`,携带无效、停用、已被替换或 `revoked:*` bearer 则返回 `401 { "error": "API token 无效或已停用。" }`;不会回显 token。
...
活动心跳必须与 D1 中的用户、session ID、client version、build ID、process nonce 和当前 sequence **全部精确匹配**。服务端不先读取租约状态作授权:它先签名请求绑定的 `sequence + 1` 候选,再由单条 D1 compare-and-swap 同时检查完整绑定元组、旧 sequence、同一用户所有租约的 `last_heartbeat_at` 最小间隔、在线行 ownership 和 `force_exit_at IS NULL`;只有 CAS 获胜才返回候选。CAS 失败后才读取提交状态,依次分类为强制退出、绑定/重放冲突、租约行不存在或过快心跳。并发同序号因此恰有一个 `200` 租约和一个 `409`,不会被 isolate 内存限流提前改成 `429`;重放、回退、跳号、绑定变更或跨用户 ownership 返回 `409`(行在但冲突=疑似篡改/顶号,客户端按安全事件强退),租约行不存在(token 仍有效,即断网/睡眠超过在线窗口被 Cron 清理)返回 `410 Gone`(会话过期,客户端提示后回登录窗重新登录),二者都不返回签名字段。签名失败发生在 CAS 之前,因此不会推进服务端 sequence。每个 token/用户的 D1 3 秒最小间隔返回 `429`,同样不签发、不推进。强制退出响应不创建新能力。
...
**强制退出触发点**:管理端「在线状态」强制下线(下一个心跳 ≤5s 收到);账号被封禁;token 被停用/轮换。版本门禁仍先执行,所以低版本心跳可先返回 426;通过版本门禁后,旧/撤销 token 心跳返回 401并且不回显 token。kick 是**瞬态**(仅当前会话),持续封禁靠 `banned` 在登录与业务层阻断。
...
**使用日志批量上传**:客户端每次用户操作(刷写/重启/传输/ROOT…)执行完成后,把记录(操作分类/标题/结果/耗时/**步骤数据**)批量上传。服务端按 `operation_kind` 分类存储,归属用户由 token 解析。必须带 `Authorization: Bearer <token>`。
  ...
  "logs": [
    {
      ...
      "title": "正在刷写 boot",
      "status": "success",
      "event_id": "1786700000123-7",
      "started_at": 1786700000,
      "ended_at": 1786700060,
      "duration_ms": 60000,
      "details": [
        { "timestamp_utc": 1786700000, "level": "Info", "message": "正在下载固件包" },
        { "timestamp_utc": 1786700030, "level": "Warning", "message": "分区 product：已跳过" },
        { "timestamp_utc": 1786700060, "level": "Success", "message": "正在刷写 boot完成。" }
      ]
    }
  ]
}
...
| `title` | string | 操作标题 |
| `status` | string | `success` / `failed` / `canceled` / `started` |
...
```bash
curl -H "X-Nwflash-Version: 1.4.0" \
     -H "Authorization: Bearer <API_TOKEN>" \
     "https://api.nwflash.cc.cd/api/rom?pd=PD2417&version=16.2.12.0.W10.V000L1"
# → 200,带真实 OTA 链接

curl -H "X-Nwflash-Version: 1.4.0" \
     -H "Authorization: Bearer <API_TOKEN>" \
     "https://api.nwflash.cc.cd/api/rom?pd=PD2417&version=99.99"
...
| 2026-09-04 | **版本门禁收紧**:受保护业务端点缺失/非法 `X-Nwflash-Version` 分别返回 `400 CLIENT_VERSION_REQUIRED` / `400 CLIENT_VERSION_INVALID`;健康、pin、遥测、崩溃、版本发现和客户端在线列表保持明确豁免;低版本仍返回 `426 UPDATE_REQUIRED` |
| 2026-09-10 | **后台补齐 V1 步骤数据展示**:`/api/usage/logs` 的 `details`(过程日志,09-02 起随 V1 使用日志上传、落 `usage_logs.details_json`)此前从未在审计视图暴露——schema-1 记录一律显示"旧客户端未上传步骤数据"。管理端 `GET /api/usage-logs/v2/runs/v1:<id>` 现返回 `details` 并以 `detail_available` 区分"已上传步骤数据/真正未上传";审计页将已上传的 details 渲染为时间线,占位提示仅保留给空/损坏记录 |
