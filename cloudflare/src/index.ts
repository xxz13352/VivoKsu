...
/** Public API route method contract. OPTIONS is handled as a global CORS preflight above. */
const ROUTE_METHODS: ReadonlyMap<string, string> = new Map([
  ["/health", "GET"],
  ["/api/security/pins", "GET"],
  ["/api/integrity/report", "POST"],
  ...
  ["/api/heartbeat", "POST"],
  ["/api/online", "GET"],
  ["/api/operation/authorize", "POST"],
  ["/api/usage/logs", "POST"],
  ["/api/usage/traces/v2", "POST"],
  ["/api/rom", "GET"],
]);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      const expectedMethod = ROUTE_METHODS.get(url.pathname);
      ...
      if (url.pathname === "/api/rom" && request.method === "GET") {
        const gate = await checkAppVersion(env, request);
        if (gate) return gate;
...
async function resolveRom(env: Env, pd: string | null, version: string | null, request: Request): Promise<Response> {
  ...
  const normalizedPd = pd?.trim() ?? "";
  const normalizedVersion = version?.trim() ?? "";
  if (!normalizedPd || !normalizedVersion) {
  ...
  try {
    resp = await fetch(upstream, {
      ...
      body: JSON.stringify({ ver: env.VOTA_VER ?? "0.1.0", pd: normalizedPd, version: normalizedVersion }),
    });
  } catch {
    await logAccess(env, userId, userName, normalizedPd, normalizedVersion, null, 502);
  ...
  if (!data.ok) {
    ...
    const upstreamError = typeof data.error === "string" ? data.error.trim() : "";
    const status = mapError(code, upstreamError || "VOTA 未能解析 ROM 下载链接。");
    // 明确区分两种失败，客户端才能给出可操作提示：
    //   404 = 没有这个固件包（机型/版本未上架或不受支持）；
    //   5xx = 服务器/上游 ROM 服务故障。
    let message: string;
    if (status === 404) {
      message =
        upstreamError && !/not found/i.test(upstreamError)
          ? `未找到该固件包：${upstreamError}`
          : "未找到该固件包（该机型/版本可能未上架或不受支持）。";
    } else if (status >= 500) {
      message = upstreamError
        ? `上游 ROM 服务返回错误：${upstreamError}`
        : "上游 ROM 服务暂时不可用。";
    } else {
      message = upstreamError || "VOTA 未能解析 ROM 下载链接。";
    }
    await logAccess(env, userId, userName, normalizedPd, normalizedVersion, null, status);
    return json({ error: message }, status);
  }

  const url = usableRomUrl(data.url);
  if (url === null) {
    await logAccess(env, userId, userName, normalizedPd, normalizedVersion, null, 502);
    return json({ error: "上游返回异常。" }, 502);
  }
  await logAccess(env, userId, userName, normalizedPd, normalizedVersion, url, 200);
  return json({
    pd: normalizedPd,
...
async function heartbeat(env: Env, request: Request): Promise<Response> {
  ...
  if (!advanced) {
    ...
    if (!state) {
      // 租约行不存在但 token 有效:是超时清理(断网/睡眠超过在线窗口),不是绑定冲突。
      // 与 409 区分:409=行在但冲突(疑似篡改/顶号,客户端按安全事件强退);
      // 410=行没了(会话过期,客户端提示后回登录窗重新登录即可)。
      return json({ error: "登录会话已过期,请重新登录。" }, 410);
    }
...
async function acceptUsageLogs(env: Env, request: Request): Promise<Response> {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return json({ error: "日志请求体不合法。" }, 400);
  }
  const logsValue = (body as Record<string, unknown>).logs;
  if (!Array.isArray(logsValue)) {
    return json({ error: "日志请求体不合法。" }, 400);
  }
  const logs = logsValue as Array<Record<string, unknown> | null>;
...
/** 版本门禁:缺失/非法头 fail closed,合法低版本 → 426,否则 null。 */
async function checkAppVersion(env: Env, request: Request): Promise<Response | null> {
  const rawVersion = request.headers.get("X-Nwflash-Version");
  if (rawVersion === null || rawVersion.trim() === "") {
    return withNoStore(json({
      error: "缺少 X-Nwflash-Version。",
      code: "CLIENT_VERSION_REQUIRED",
    }, 400));
  }

  const current = rawVersion.trim();
  if (!isClientVersion(current)) {
    return withNoStore(json({
      error: "X-Nwflash-Version 格式无效。",
  ...
  if (compareVersions(current, policy.min_version) < 0) {
    return withNoStore(json({
...
function mapError(code: string | null, error: string): number {
  ...
  if (code === "UPDATE_REQUIRED") return 426;
  return 502;
}

function usableRomUrl(value: unknown): string | null {
...
function methodNotAllowed(expectedMethod: string): Response {
  return json({ error: "方法不允许。" }, 405, { Allow: `${expectedMethod}, OPTIONS` });
}
