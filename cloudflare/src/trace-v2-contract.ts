...
/** 旧版(V1 使用日志)客户端上传的过程日志行,来源为 usage_logs.details_json。 */
export interface LegacyTraceDetailLineV2 { timestamp_utc: number; level: string; message: string; }
export interface TraceRunDetailV2 {
  ...
  events: TraceEventV2[];
  /** 仅 source_schema = 1 时可能非空:旧客户端随使用日志一并上传的步骤数据。 */
  details?: LegacyTraceDetailLineV2[];
}
