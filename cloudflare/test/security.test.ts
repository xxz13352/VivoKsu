...
describe("signed lease routes", () => {
  ...
  it("returns 410 Gone for an unknown session without returning a signed lease", async () => {
    // 租约行不存在(含从未登录)且 token 有效 → 410 会话过期,区别于 409 绑定冲突。
  ...
  it("returns 410 Gone for a purged lease with a valid token instead of 409", async () => {
    // 断网/睡眠超过在线窗口后,租约行被清理;token 仍有效。此时返回 410(会话过期,
    // 客户端提示后回登录窗)而非 409(行在但冲突=疑似篡改,客户端强退)。
    const { env, db, token } = await testEnv({ token: "purged-lease-token" });
    expect((await postLogin(env)).status).toBe(200);
    db.sessionLeases.delete("session-abc");
