...
const GATED_ROUTE_CASES = [
  {
    ...
    body: {
      ...
      build_id: "build-gate",
      process_nonce: "nonce-gate",
      session_id: "gate-login-session",
    },
    validStatus: 401,
  },
  { name: "me", method: "GET", path: "/api/me", validStatus: 200 },
  { name: "heartbeat", method: "POST", path: "/api/heartbeat", validStatus: 401 },
  { name: "operation authorize", method: "POST", path: "/api/operation/authorize", validStatus: 401 },
  { name: "usage logs", method: "POST", path: "/api/usage/logs", validStatus: 401 },
  { name: "trace upload", method: "POST", path: "/api/usage/traces/v2", validStatus: 401 },
  { name: "rom", method: "GET", path: "/api/rom?pd=PD-GATE&version=1.0.0", validStatus: 401 },
] as const;
const VERSION_GATE_POLICY = {
  version: "2.0.0",
  min_version: "1.4.0",
  download_url: "https://example.test/nwflash-2.0.0.zip",
};
...
describe("fail-closed client version gate", () => {
  beforeEach(async () => {
    await env.DB.prepare(
      "INSERT INTO app_versions (version, min_version, download_url, enabled) VALUES (?, ?, ?, 1)",
    ).bind(VERSION_GATE_POLICY.version, VERSION_GATE_POLICY.min_version, VERSION_GATE_POLICY.download_url).run();
  });

  it("rejects missing, blank, malformed, and low versions before every gated route", async () => {
    const variants = [
      { label: "missing", version: undefined, status: 400, code: "CLIENT_VERSION_REQUIRED" },
      { label: "blank", version: "", status: 400, code: "CLIENT_VERSION_REQUIRED" },
      { label: "malformed", version: "1.4.0 with spaces", status: 400, code: "CLIENT_VERSION_INVALID" },
      { label: "illegal character", version: "1.4.0!", status: 400, code: "CLIENT_VERSION_INVALID" },
      { label: "too long", version: "1." + "0".repeat(32), status: 400, code: "CLIENT_VERSION_INVALID" },
      { label: "low", version: "1.3.9", status: 426, code: "UPDATE_REQUIRED" },
    ] as const;

    for (const route of GATED_ROUTE_CASES) {
      for (const variant of variants) {
        const response = await fetchGateRoute(route, variant.version);
        const body = await response.json() as Record<string, unknown>;
        expect(response.status, `${route.name}/${variant.label}`).toBe(variant.status);
        expect(body.code, `${route.name}/${variant.label}`).toBe(variant.code);
        if (variant.version) {
          expect(JSON.stringify(body), `${route.name}/${variant.label}`).not.toContain(variant.version);
        }
        expect(response.headers.get("access-control-allow-origin"), `${route.name}/${variant.label}`).toBe("*");
        expect(response.headers.get("cache-control"), `${route.name}/${variant.label}`).toBe("no-store");
        if (variant.status === 426) {
          expect(body).toMatchObject({
            latest: VERSION_GATE_POLICY.version,
            min: VERSION_GATE_POLICY.min_version,
            download_url: VERSION_GATE_POLICY.download_url,
          });
        }
      }
    }
    ...
    expect(await scalar("SELECT COUNT(*) AS value FROM access_logs")).toBe(0);
  });

  it("passes equal and higher versions through to each route's existing behavior", async () => {
    for (const version of [VERSION_GATE_POLICY.min_version, "9.9.9"]) {
      for (const route of GATED_ROUTE_CASES) {
        const response = await fetchGateRoute(route, version);
        expect(response.status, `${route.name}/${version}`).toBe(route.validStatus);
        expect(response.status, `${route.name}/${version}`).not.toBe(400);
        expect(response.status, `${route.name}/${version}`).not.toBe(426);
      }
    }
  });

  it("keeps missing and malformed headers fail-closed even when no policy is enabled", async () => {
    await env.DB.prepare("DELETE FROM app_versions").run();

    for (const version of [undefined, "", "bad version"]) {
      const response = await fetchGateRoute(GATED_ROUTE_CASES[1], version);
      const body = await response.json() as Record<string, unknown>;
      expect(response.status).toBe(400);
      expect(body.code).toBe(version === "bad version" ? "CLIENT_VERSION_INVALID" : "CLIENT_VERSION_REQUIRED");
      expect(response.headers.get("cache-control")).toBe("no-store");
    }

    const valid = await fetchGateRoute(GATED_ROUTE_CASES[1], "1.4.0");
    expect(valid.status).toBe(200);
    expect(await valid.json()).toEqual({ loggedIn: false });
  });

  it("does not apply the gate to health, bootstrap, telemetry, crash, version discovery, online, or preflight", async () => {
    const options = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/health", { method: "OPTIONS" }), env);
    expect(options.status).toBe(204);

    const health = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/health"), env);
    expect(health.status).toBe(200);

    const pins = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/api/security/pins"), env);
    expect(pins.status).toBe(200);

    const integrity = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/api/integrity/report", {
      method: "POST",
      headers: { "Content-Type": "application/json", "CF-Connecting-IP": "198.51.100.71" },
      body: JSON.stringify({
        event_id: "gate-integrity-event",
        phase: "startup",
    ...
    expect(integrity.status).toBe(202);

    const crash = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/api/diagnostics/crash", {
      method: "POST",
      headers: { "Content-Type": "application/json", "CF-Connecting-IP": "198.51.100.72" },
      body: JSON.stringify({
        event_id: "gate-crash-event",
        client_version: "1.4.0",
        build_id: "build-gate",
        session_id: "session-gate",
        panic_message: "gate test",
        backtrace: "",
        occurred_at: Math.floor(Date.now() / 1000),
      }),
    }), env);
    ...
    const appVersion = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/api/app/version"), env);
    expect(appVersion.status).toBe(200);

    const online = await apiWorker.fetch(new Request("https://api.nwflash.cc.cd/api/online"), env);
    expect(online.status).toBe(401);
    const onlineBody = await online.json() as Record<string, unknown>;
    expect(onlineBody.error).toBe("请先登录。");
  });
});

async function fetchGateRoute(
  route: (typeof GATED_ROUTE_CASES)[number],
  version: string | undefined,
): Promise<Response> {
  const headers = new Headers({ "Content-Type": "application/json" });
  if (version !== undefined) headers.set("X-Nwflash-Version", version);
  const init: RequestInit = { method: route.method, headers };
  if (route.body !== undefined) init.body = JSON.stringify(route.body);
  return apiWorker.fetch(new Request(`https://api.nwflash.cc.cd${route.path}`, init), env);
}
