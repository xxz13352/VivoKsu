...
describe("administrator trace V2 API", () => {
  ...
  it("returns explicit legacy detail unavailability", async () => {
    ...
    expect(response.status).toBe(200);
    const detail = await response.json() as TraceRunDetailV2;
  ...
  it("surfaces uploaded legacy step details for V1 runs", async () => {
    const details = [
      { timestamp_utc: 1_787_500_000, level: "Info", message: "正在下载固件包" },
      { timestamp_utc: 1_787_500_030, level: "Warning", message: "分区 product：已跳过" },
      { timestamp_utc: 1_787_500_060, level: "Success", message: "Legacy flash完成。" },
    ];
    await env.DB.prepare(
      `INSERT INTO usage_logs
         (id, api_user_id, api_user_name, operation_kind, title, status, event_key, started_at, details_json)
       VALUES (43, 7, 'Alice Zhang', 'fastboot_flash', 'Legacy flash with steps', 'success', 'legacy-43', ?, ?)`,
    )
      .bind(Math.floor(SAME_STARTED_AT_MS / 1000), JSON.stringify(details))
      .run();

    const response = await adminGet("/api/usage-logs/v2/runs/v1%3A43");
    ...
    expect(detail).toMatchObject({
      ...
      run: { source_schema: 1, trace_ref: "v1:43", legacy_id: 43 },
      events: [],
    });
    expect(detail.details).toEqual(details);
  });

  it("treats malformed stored legacy details as absent step data", async () => {
    await env.DB.prepare(
      `INSERT INTO usage_logs
         (id, api_user_id, api_user_name, operation_kind, title, status, event_key, started_at, details_json)
       VALUES (44, 7, 'Alice Zhang', 'fastboot_flash', 'Corrupt details row', 'failed', 'legacy-44', ?, 'not-json')`,
    ).bind(Math.floor(SAME_STARTED_AT_MS / 1000)).run();

    const response = await adminGet("/api/usage-logs/v2/runs/v1%3A44");

    expect(response.status).toBe(200);
    const detail = await response.json() as TraceRunDetailV2;
    expect(detail.detail_available).toBe(false);
    expect(detail.detail_unavailable_reason).toBe("legacy_client_no_step_data");
    expect(detail.details).toEqual([]);
  });
  ...
  it("exports the current run filters as audited NDJSON", async () => {
    ...
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(response.headers.get("x-nwflash-export-version")).toBeNull();
  ...
  it.each([0, 1, 200, 205])("emits one completion marker for reliable export size %i", async (count) => {
    if (count > 0) await seedRunsAtSameTimestamp(count, SAME_STARTED_AT_MS);

    const response = await adminReliableGet("/api/usage-logs/v2/export");
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("application/x-ndjson");
    expect(response.headers.get("x-nwflash-export-version")).toBe("1");
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(response.headers.get("content-disposition")).toMatch(/^attachment; filename="nwflash-traces-[0-9]+\.ndjson"$/);
    expect(marker).toEqual({ _nwflash_export: { version: 1, type: "complete", rows: count } });
    expect(lines).toHaveLength(count);
    expect(lines.some((line) => "_nwflash_export" in line)).toBe(false);
  });

  it("rejects reliable export without the CSRF header before writing an audit row", async () => {
    const response = await adminWorker.fetch(new Request(
      "https://web.nwflash.cc.cd/api/usage-logs/v2/export",
      { headers: { Cookie: `nwflash_session=${ADMIN_SESSION_TOKEN}`, Accept: RELIABLE_EXPORT_ACCEPT } },
    ), env);
    const body = await response.json() as Record<string, any>;

    expect(response.status).toBe(403);
    expect(response.headers.get("x-nwflash-export-version")).toBeNull();
    expect(body).toMatchObject({ ok: false, error: { code: "TRACE_FORBIDDEN" } });
    expect(await scalar(
      "SELECT COUNT(*) AS value FROM admin_audit_log WHERE action = 'export_trace'",
    )).toBe(0);
  });

  it("keeps reliable export authentication in the frozen 401 envelope", async () => {
    const response = await adminWorker.fetch(new Request(
      "https://web.nwflash.cc.cd/api/usage-logs/v2/export",
      {
        headers: {
          Accept: RELIABLE_EXPORT_ACCEPT,
    ...
    expect(response.headers.get("x-nwflash-export-version")).toBeNull();
    expect(body).toMatchObject({ ok: false, error: { code: "TRACE_UNAUTHORIZED" } });
  });

  it("does not emit a completion marker when a later reliable export batch fails", async () => {
    await seedRunsAtSameTimestamp(205, SAME_STARTED_AT_MS);

    const response = await adminReliableGet("/api/usage-logs/v2/export");
    // The first batch has already been selected before the Response is
    // returned.  Break the next keyset query to model a mid-stream D1 error.
    await env.DB.exec("DROP TABLE usage_operation_runs");
    ...
    expect(text).not.toContain('"_nwflash_export"');
    expect(await scalar(
      "SELECT COUNT(*) AS value FROM admin_audit_log WHERE action = 'export_trace'",
    )).toBe(1);
  });
...
const RELIABLE_EXPORT_ACCEPT = 'application/x-ndjson; profile="nwflash-export-v1"';

async function adminReliableGet(path: string): Promise<Response> {
  return adminRequest(path, {
    headers: {
      Accept: RELIABLE_EXPORT_ACCEPT,
      "X-Requested-With": "XMLHttpRequest",
