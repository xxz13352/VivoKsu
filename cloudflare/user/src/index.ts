...
async function sessions(env: Env, user: AuthUser): Promise<Response> {
  ...
  const serialized = rows.results.map((row) => {
    ...
    return {
      ...
      // 会话时间与门户活动时间统一为北京时间展示;此前裸 UTC 的 ISO 串
      // 被用户读成"慢 8 小时"。
      connectedAt: formatBeijingTime(row.connected_at),
      lastSeenAt: formatBeijingTime(row.last_seen_at),
...
/** epoch 秒 → 北京时间 `yy/M/d HH:mm`(如 26/9/14 16:46)。 */
function formatBeijingTime(epochSeconds: number): string {
  const shifted = new Date((epochSeconds + 8 * 3600) * 1000);
  if (Number.isNaN(shifted.getTime())) return "—";
  const pad = (value: number) => String(value).padStart(2, "0");
  return `${String(shifted.getUTCFullYear()).slice(-2)}/${shifted.getUTCMonth() + 1}/${shifted.getUTCDate()} ${pad(shifted.getUTCHours())}:${pad(shifted.getUTCMinutes())}`;
}
