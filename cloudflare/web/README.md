...
### Version and user mutation authority

Administrator version/user mutations use exact same-origin routes:

| Route | Method | Success | Validation / authority |
| --- | --- | --- | --- |
| `/api/app-versions` | `POST` | `201` | Missing/mistyped fields return `400`; duplicate version returns `409` |
| `/api/app-versions/{id}` | `PUT`, `DELETE` | `200` | Canonical positive safe-integer ID; empty/invalid update returns `400`; missing target returns `404` |
| `/api/users` | `POST` | `201` | Missing/mistyped fields or password shorter than 6 return `400`; duplicate username returns `409` |
| `/api/users/{id}` | `PUT`, `DELETE` | `200` | Canonical positive safe-integer ID; empty/invalid update returns `400`; missing target returns `404` |
| `/api/users/{id}/rotate-token` | `POST` | `200` | Missing target returns `404`; token is returned only after an authoritative update |

Item routes reject extra path segments. All fields in a multi-field update are validated before one atomic `UPDATE`; a bad `min_version`, short `newPassword`, or unknown-only body cannot partially modify a row. Errors keep the existing `{ "error": "..." }` administrator envelope, and every mutation still requires the administrator cookie plus `X-Requested-With: XMLHttpRequest`.
