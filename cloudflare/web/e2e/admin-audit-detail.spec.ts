...
async function installDetailApi(page: Page, exportedUrls: string[]) {
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/usage-logs/v2/export") {
      // Let the controlled HTTP server validate the streaming export request
      // and emit the negotiated completion marker.
...
async function createControlledExportServer() {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/usage-logs/v2/export") {
      if (
        request.method !== "GET"
        || request.headers.accept !== 'application/x-ndjson; profile="nwflash-export-v1"'
        || request.headers["x-requested-with"] !== "XMLHttpRequest"
      ) {
        response.writeHead(403, { "Content-Type": "application/json; charset=utf-8" });
        response.end(JSON.stringify({ error: "missing reliable export headers" }));
        return;
      }
      ...
      response.end(`${JSON.stringify({ trace_ref: TRACE_REF })}\n${JSON.stringify({
        _nwflash_export: { version: 1, type: "complete", rows: 1 },
      })}\n`);
      return;
    }
...
test("uses negotiated streaming NDJSON download without route or output identifiers in the export query", async ({ page }) => {
  ...
  try {
    ...
    const exported = new URL(server.exportedUrls[0]);
    ...
    expect(server.exportedUrls).toHaveLength(1);
  } finally {
    await server.close();
  }
});
