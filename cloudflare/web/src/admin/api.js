...
export function createApiClient({ fetchImpl = globalThis.fetch, onUnauthorized = () => {} } = {}) {
  ...
  /**
   * Opens a bounded, same-origin streaming response without consuming a
   * successful body.  Non-success responses use the same error envelope as
   * request(), but their body is capped so an upstream cannot force an
   * unbounded error allocation.
   */
  async function requestStream(path, options = {}) {
    ...
    headers.set("X-Requested-With", "XMLHttpRequest");
    headers.set("Accept", EXPORT_ACCEPT);
    ...
    if (!response.ok) {
      const parsed = await parseBoundedResponse(response);
      const error = normalizeHttpError(response.status, parsed, collectSecrets(options.body));
      if (response.status === 401) {
        try {
          onUnauthorized(error);
        } catch {
          // Session cleanup is best-effort and must not hide the API error.
        }
      }
      throw error;
    }
  ...
  return Object.freeze({
    ...
    openTraceExport: (query = {}, options = {}) => requestStream(
      "/api/usage-logs/v2/export",
...
async function parseBoundedResponse(response) {
  const text = await readBoundedResponseText(response, MAX_ERROR_BODY_BYTES);
  if (text === null || text === "") return { value: null, invalidJson: false };
