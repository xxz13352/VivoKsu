...
import { formatBeijingTime } from "../format-time.js";
...
const EXPORT_ACCEPT = 'application/x-ndjson; profile="nwflash-export-v1"';
const EXPORT_PROTOCOL_VERSION = "1";
const MAX_EXPORT_BYTES = 32 * 1024 * 1024;
const MAX_EXPORT_LINE_BYTES = 1 * 1024 * 1024;
...
export function createAuditPage(context = {}) {
  ...
  function stopExport() {
    ...
    void exportReader?.cancel().catch(() => {});
    exportController = null;
    exportReader = null;
    exportCancelButton?.remove();
    exportCancelButton = null;
    if (exportObjectUrl) {
      try { window.URL?.revokeObjectURL?.(exportObjectUrl); } catch { /* best effort */ }
  ...
  function renderRunDetail(detail) {
    const { run } = detail;
    const content = createElement(document, "div", { className: "audit-content" });
    const subtitle = run.source_schema === 1
      ? "旧版记录展示客户端随使用日志上传的过程日志。"
      : "按服务端 sequence 展示已持久化步骤。";
    content.append(renderBreadcrumbs("run"), renderLevelHeading(provided(run.title), subtitle));
    content.append(createAuditFilterForm(), createRunSummary(run), createExportButton());

    if (run.source_schema === 1 || detail.source_schema === 1) {
      const legacyLines = normalizeLegacyDetails(detail.details);
      if (legacyLines.length > 0) {
        content.append(createLegacyDetailSection(legacyLines));
      } else {
  ...
  /** 旧客户端(V1 使用日志)随操作日志一并上传的过程日志时间线。 */
  function createLegacyDetailSection(lines) {
    const list = createElement(document, "ol", { className: "audit-event-list", "aria-label": "旧版步骤数据" });
    lines.forEach((line, index) => {
      list.append(createElement(document, "li", {}, [
        createElement(document, "code", { "data-legacy-detail-index": String(index + 1) }, String(index + 1)),
        legacyLevelBadge(document, line.level),
        createElement(document, "span", {}, `时间：${formatTime(line.timestamp_utc * 1000)}`),
        createElement(document, "span", {}, provided(line.message)),
      ]));
    });
    return createElement(document, "section", { className: "audit-evidence", "aria-label": "旧版步骤数据" }, [
      createElement(document, "h3", {}, "旧版步骤数据"),
      createElement(document, "p", { className: "muted" }, "该记录来自旧客户端使用日志,过程日志按上传顺序展示。"),
      list,
    ]);
  }
  ...
  function createExportButton() {
    ...
    listen(button, "click", () => { void exportCurrent(button); });
    return button;
  }

  async function exportCurrent(button) {
    if (button.disabled || exportPending) return;
    exportPending = true;
    button.disabled = true;
    button.textContent = "正在准备导出…";
    const generation = activation;
    const pageSignal = controller?.signal;
    const operation = new window.AbortController();
    let cancelRequested = false;
    let completed = false;
    let cancelButton = null;
    const abortFromPage = () => operation.abort();
    exportController = operation;
    pageSignal?.addEventListener("abort", abortFromPage, { once: true });

    cancelButton = createElement(document, "button", {
      type: "button",
      className: "button",
      "data-audit-action": "cancel-export",
      "aria-label": "取消导出",
    }, "取消导出");
    ...
    (button.parentElement ?? element).append(cancelButton);

    try {
      requireApi("openTraceExport");
      const response = await api.openTraceExport(
        buildQuery(currentRoute, EXPORT_FILTERS, false),
        { signal: operation.signal },
      );
      if (!isCurrent(generation, pageSignal) || operation.signal.aborted) return;
      const payload = await readExportResponse(response, operation.signal, (reader) => {
        exportReader = reader;
      });
      if (!isCurrent(generation, pageSignal) || operation.signal.aborted) return;
      exportObjectUrl = await triggerExportDownload(payload.chunks, window, document);
      completed = true;
      button.textContent = "导出完成，再次导出";
      context.announce?.(`审计导出已完成（${payload.rows} 行）。`, { kind: "success" });
    } catch (error) {
      if (!isCurrent(generation, pageSignal)) return;
      if (cancelRequested || isAbort(error) || operation.signal.aborted) {
        button.textContent = "导出已取消，再次导出";
        context.announce?.("审计导出已取消。");
      } else {
        button.textContent = "重试导出";
        context.alert?.(safeExportErrorMessage(error), { title: "导出失败" });
      }
    } finally {
      pageSignal?.removeEventListener("abort", abortFromPage);
      if (exportReader) {
        try { await exportReader.cancel(); } catch { /* best effort */ }
        try { exportReader.releaseLock(); } catch { /* best effort */ }
        exportReader = null;
      }
      if (exportController === operation) exportController = null;
      if (exportCancelButton === cancelButton) exportCancelButton = null;
      cancelButton?.remove();
      if (exportObjectUrl) {
        try { window.URL?.revokeObjectURL?.(exportObjectUrl); } catch { /* best effort */ }
        exportObjectUrl = null;
      }
      if (isCurrent(generation, pageSignal)) {
        exportPending = false;
        button.disabled = false;
        if (!completed && !cancelRequested && button.textContent === "正在准备导出…") {
          button.textContent = "重试导出";
        }
      }
    }
...
class ExportProtocolError extends Error {
  constructor(message) {
    super(message);
    this.name = "ExportProtocolError";
    this.kind = "invalid_response";
    this.code = "ADMIN_EXPORT_INVALID";
  }
}

async function readExportResponse(response, signal, onReader = () => {}) {
  if (!response?.ok) throw new ExportProtocolError("导出响应状态无效。");
  const contentType = response.headers?.get("Content-Type")?.toLowerCase() ?? "";
  if (!contentType.includes("application/x-ndjson")) {
    throw new ExportProtocolError("导出响应类型无效。");
  }
  if (response.headers?.get("X-Nwflash-Export-Version") !== EXPORT_PROTOCOL_VERSION) {
    throw new ExportProtocolError("导出协议版本无效。");
  }
  if (!response.body?.getReader) throw new ExportProtocolError("浏览器不支持导出流。");
  ...
  const processLine = (line) => {
    if (line.length === 0) throw new ExportProtocolError("导出包含空行。");
    if (encoder.encode(line).byteLength > MAX_EXPORT_LINE_BYTES) {
      throw new ExportProtocolError("导出单行超过大小限制。");
    }
    let value;
    try {
      value = JSON.parse(line);
    } catch {
      throw new ExportProtocolError("导出包含无效 JSON。");
    }
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new ExportProtocolError("导出行结构无效。");
    }

    if (Object.prototype.hasOwnProperty.call(value, "_nwflash_export")) {
      if (marker !== null) throw new ExportProtocolError("导出完成标记重复。");
      const candidate = value._nwflash_export;
      if (
        !candidate
        ...
        throw new ExportProtocolError("导出完成标记无效。");
      }
      marker = candidate;
      return;
    }

    if (marker !== null) throw new ExportProtocolError("导出完成标记不是最后一行。");
    const encoded = encoder.encode(`${line}\n`);
    bytes += encoded.byteLength;
    if (bytes > MAX_EXPORT_BYTES) throw new ExportProtocolError("导出超过 32 MiB 大小限制。");
    chunks.push(`${line}\n`);
    rows += 1;
  };
  ...
  try {
    while (true) {
      if (signal?.aborted) throw abortExportError();
      const next = await reader.read();
      if (next.done) {
        done = true;
        break;
      }
      if (!next.value || typeof next.value.byteLength !== "number") {
        throw new ExportProtocolError("导出流数据无效。");
      }
      ...
      if (encoder.encode(lineBuffer).byteLength > MAX_EXPORT_LINE_BYTES) {
        throw new ExportProtocolError("导出单行超过大小限制。");
      }
    }
    lineBuffer += decoder.decode();
    if (lineBuffer.length > 0) {
      processLine(lineBuffer.endsWith("\r") ? lineBuffer.slice(0, -1) : lineBuffer);
    }
    if (marker === null) throw new ExportProtocolError("导出缺少完成标记。");
    if (marker.rows !== rows) throw new ExportProtocolError("导出行数与完成标记不一致。");
    return { chunks, rows, bytes };
  } finally {
    if (!done) {
...
async function triggerExportDownload(chunks, window, document) {
  if (typeof window?.Blob !== "function") throw new ExportProtocolError("浏览器不支持 Blob 下载。");
  const blob = new window.Blob(chunks, { type: "application/x-ndjson;charset=utf-8" });
  const objectUrl = window.URL?.createObjectURL?.(blob);
  if (!objectUrl) throw new ExportProtocolError("浏览器不支持安全下载。");
  const anchor = document.createElement("a");
  anchor.href = objectUrl;
  anchor.download = "nwflash-traces.ndjson";
  anchor.hidden = true;
  ...
  try {
    ...
    try { window.URL?.revokeObjectURL?.(objectUrl); } catch { /* best effort */ }
    throw error;
  } finally {
    anchor.remove();
  }
...
function safeExportErrorMessage(error) {
  if (error instanceof ExportProtocolError) return error.message;
...
function requireRunDetail(value, route) {
  ...
  if (value.source_schema === 1) {
    // 旧版行:details_json 里存有过程日志时 detail_available 为 true,否则保持历史占位语义。
    if (typeof value.detail_available !== "boolean" || value.run.run_id !== null || value.events.length !== 0) {
      throw new Error("旧版运行详情响应格式无效。");
    }
    if (value.detail_available === true
      && (value.detail_unavailable_reason !== null || normalizeLegacyDetails(value.details).length === 0)) {
      throw new Error("旧版运行详情响应格式无效。");
    }
    if (value.detail_available === false && value.detail_unavailable_reason !== "legacy_client_no_step_data") {
      throw new Error("旧版运行详情响应格式无效。");
    }
  } else {
    if (value.detail_available !== true || value.run.run_id !== route.runId.slice(3)) {
...
/** 客户端使用日志的 PascalCase 级别闭集(与 OperationLogLevel 序列化一致)。 */
const LEGACY_DETAIL_LEVELS = new Set(["info", "success", "warning", "error"]);

/**
 * 旧版步骤行的级别标签。与 statusBadge 的运行/事件状态枚举不同,这里展示客户端
 * 使用日志的 Info/Success/Warning/Error;未知级别原样大写展示——文本节点无注入面,
 * className 只经冻结小写映射构造,不接纳原始字符串。
 */
function legacyLevelBadge(document, level) {
  const raw = typeof level === "string" && level.length > 0 ? level : "Info";
  const normalized = LEGACY_DETAIL_LEVELS.has(raw.toLowerCase()) ? raw.toLowerCase() : "unknown";
  return createElement(document, "span", {
    className: `audit-status audit-status-${normalized}`,
    "data-status": normalized,
  }, raw.toUpperCase());
...
/** 防御性归一化旧版(V1 使用日志)过程日志行;无效成员丢弃,非数组输入返回空。 */
function normalizeLegacyDetails(value) {
  if (!Array.isArray(value)) return [];
  const lines = [];
  for (const item of value) {
    if (!item || typeof item !== "object") continue;
    const timestamp = Number(item.timestamp_utc);
    const message = typeof item.message === "string" ? item.message : "";
    if (!Number.isSafeInteger(timestamp) || timestamp < 0 || message.length === 0) continue;
    lines.push({
      timestamp_utc: timestamp,
      level: typeof item.level === "string" && item.level.length > 0 ? item.level : "Info",
      message,
    });
  }
  return lines;
