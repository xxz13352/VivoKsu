...
import { formatBeijingUtcText } from "../format-time.js";
...
export function createUsersPage(context) {
  ...
  async function load() {
    ...
    try {
      ...
      if (tokenRecovery?.userId !== null && tokenRecovery?.userId !== undefined
        && !users.some((user) => String(user.id) === tokenRecovery.userId)) {
        tokenRecovery = null;
      }
      render();
      return true;
  ...
  function render() {
    ...
    const recoveryNotice = tokenRecovery ? createElement(context.document, "div", {
      className: "workspace-notice",
      role: "alert",
      "data-token-recovery": tokenRecovery.kind,
    }, [
      createElement(context.document, "strong", {}, tokenRecovery.kind === "create" ? "创建结果待确认" : "轮换结果待确认"),
      createElement(context.document, "p", {}, tokenRecovery.kind === "create"
        ? "服务器未返回有效的一次性令牌。已发起权威刷新，请确认用户是否存在后，在用户行手动轮换令牌。"
        : "服务器未返回有效的一次性令牌。已发起权威刷新，请在用户行手动再次轮换；不会自动重复请求。"),
      createElement(context.document, "button", {
        type: "button",
        className: "button",
        "data-action": "refresh-token-recovery",
      }, "刷新用户列表"),
    ]) : null;
    recoveryNotice?.querySelector('[data-action="refresh-token-recovery"]')?.addEventListener("click", () => {
    ...
    element.replaceChildren(createElement(context.document, "div", { className: "workspace-card", "data-page-state": pageState }, [
      ...
      staleNotice, recoveryNotice, tokenHost, createElement(context.document, "div", { className: "workspace-filters" }, [search, statusFilter]), createForm, list,
    ]));
  ...
  async function createUser(body, button) {
    ...
    try {
      ...
      const validated = requireCreateUserResponse(result);
      oneTimeToken = validated.token;
      tokenGeneration = request.generation;
      tokenUserId = String(validated.id);
      tokenAwaitingReload = true;
      tokenRecovery = null;
      ...
      if (!isCurrent(request)) return;
      if (isInvalidMutationResponse(error)) {
        await recoverMalformedMutation(request, "create", null);
        if (isCurrent(request)) {
          context.alert?.("创建结果待确认：未收到有效的一次性令牌。请刷新确认后手动轮换令牌。", { title: "创建结果待确认" });
        }
        return;
      }
      pendingUsers.delete(key);
      button.disabled = false;
      context.alert?.(error?.message ?? "创建用户失败。", { title: "创建失败" });
    }
  }
  ...
  function userRow(user) {
    ...
    const rotate = confirmedAction({ user, action: "rotate-token", label: "轮换令牌" }, async (request) => {
      ...
      if (!isCurrent(request)) return;
      const validated = requireRotateUserTokenResponse(result);
      oneTimeToken = validated.token;
    ...
    return createElement(context.document, "li", { className: "user-row" }, [
      ...
      createElement(context.document, "time", { className: "muted" }, formatBeijingUtcText(user.created_at, String(user.created_at ?? ""))),
      ...
      void context.confirm({
        ...
        onConfirm: async () => {
          ...
          try { await mutation(request); }
            ...
            if (action === "rotate-token" && isCurrent(request) && isInvalidMutationResponse(error)) {
              await recoverMalformedMutation(request, "rotate", userKey);
              if (isCurrent(request)) {
                restore();
                context.alert?.("轮换结果待确认：未收到有效的一次性令牌。请在用户行手动再次轮换。", { title: "轮换结果待确认" });
                return true;
              }
            }
            if (isCurrent(request)) restore();
            throw error;
          }
          if (!isCurrent(request)) return true;
          if (clearToken && tokenUserId === userKey) clearOneTimeToken();
          if (clearToken && tokenRecovery?.userId === userKey) tokenRecovery = null;
  ...
  async function recoverMalformedMutation(request, kind, userKey) {
    if (!isCurrent(request)) return false;
    clearOneTimeToken();
    tokenRecovery = { kind, userId: userKey };
    pendingUsers.delete(kind === "create" ? "create" : userKey);
    awaitingAuthorityUsers.add(kind === "create" ? "create" : userKey);
    render();
    await load();
    return true;
  }
...
function requireCreateUserResponse(value) {
  if (value?.ok !== true || !isSafePositiveInteger(value?.id) || !isNonEmptyToken(value?.token)) {
    throw invalidMutationResponse("服务器未返回有效的用户 ID 和一次性令牌。");
  }
  return { id: value.id, token: value.token };
}

function requireRotateUserTokenResponse(value) {
  if (value?.ok !== true || !isNonEmptyToken(value?.token)) {
    throw invalidMutationResponse("服务器未返回有效的一次性令牌，轮换结果待确认。");
  }
...
function invalidMutationResponse(message) {
...
function isInvalidMutationResponse(error) {
  return error?.kind === "invalid_response" && error?.code === "ADMIN_INVALID_RESPONSE";
}
