...
describe("admin API client", () => {
  ...
  it.each([
    [400, "ADMIN_BAD_REQUEST", "更新参数无效。"],
    [404, "ADMIN_NOT_FOUND", "用户不存在。"],
    [409, "ADMIN_CONFLICT", "登录账号已存在。"],
  ])("keeps ordinary CRUD %i errors compatible with the admin client", async (status, code, message) => {
    const onUnauthorized = vi.fn();
    const api = createApiClient({
      fetchImpl: vi.fn().mockResolvedValue(jsonResponse({ error: message }, { status })),
      onUnauthorized,
    });

    await expect(api.updateUser(7, { enabled: false })).rejects.toMatchObject({
      kind: "http",
      status,
      code,
      message,
  ...
  it("opens the negotiated export stream with same-origin, no-store, redirect-error, and CSRF headers", async () => {
    ...
    const response = await api.openTraceExport({ status: "failed", q: "device stopped" });
    expect(await response.text()).toBe("stream-body");
    expect(fetchImpl).toHaveBeenCalledWith(
      "/api/usage-logs/v2/export?status=failed&q=device+stopped",
    ...
    expect(headers.get("Accept")).toBe('application/x-ndjson; profile="nwflash-export-v1"');
    expect(headers.get("X-Requested-With")).toBe("XMLHttpRequest");
  });

  it("normalizes a streaming export 401 and invokes centralized unauthorized cleanup", async () => {
    const onUnauthorized = vi.fn();
    const api = createApiClient({
      fetchImpl: vi.fn().mockResolvedValue(jsonResponse({
        ok: false,
        error: { code: "TRACE_UNAUTHORIZED", message: "会话已过期", request_id: "export-401" },
      }, { status: 401 })),
      onUnauthorized,
    });

    await expect(api.openTraceExport()).rejects.toMatchObject({
      kind: "unauthorized",
      status: 401,
      code: "TRACE_UNAUTHORIZED",
      requestId: "export-401",
      message: "会话已过期",
    });
    expect(onUnauthorized).toHaveBeenCalledOnce();
  });

  it("caps a streaming export error body before normalizing it", async () => {
    const huge = "x".repeat(128 * 1024);
    const api = createApiClient({
      fetchImpl: vi.fn().mockResolvedValue(new Response(huge, {
        status: 500,
        headers: { "Content-Type": "text/plain", "Content-Length": String(huge.length) },
      })),
    });

    const error = await api.openTraceExport().catch((value) => value);
    expect(error).toMatchObject({ kind: "server", status: 500, code: "ADMIN_SERVER_ERROR" });
    expect(error.message).toBe("服务器暂时无法处理请求。");
