...
function createHarness(api, extras = {}) {
  ...
  const createObjectURL = vi.fn(() => `blob:admin-export-${doms.length}`);
  const revokeObjectURL = vi.fn();
  Object.defineProperty(dom.window.URL, "createObjectURL", { configurable: true, value: createObjectURL });
  Object.defineProperty(dom.window.URL, "revokeObjectURL", { configurable: true, value: revokeObjectURL });
  doms.push(dom);
  ...
  return { dom, navigate, download, announce, alert, page, createObjectURL, revokeObjectURL };
}

function exportResponse(lines, { status = 200, headers = {}, stream = null } = {}) {
  ...
  return new Response(body, {
    ...
    headers: {
      ...
      "X-Nwflash-Export-Version": "1",
      ...headers,
    },
  });
}

function completionMarker(rows, { topExtra = {}, markerExtra = {} } = {}) {
  return `${JSON.stringify({
    _nwflash_export: { version: 1, type: "complete", rows, ...markerExtra },
    ...topExtra,
  })}\n`;
}

function userPageApi(overrides = {}) {
  return {
    getTraceUsers: vi.fn().mockResolvedValue({ items: [], next_cursor: null }),
    openTraceExport: vi.fn(),
    ...overrides,
  };
}
...
describe("audited NDJSON export", () => {
  it("streams and validates a complete export before creating a Blob download", async () => {
    const row = JSON.stringify({ trace_ref: TRACE_REF, title: "persisted row" }) + "\n";
    const api = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(
      exportResponse(`${row}${completionMarker(1)}`),
    ) });
    const { page, dom, announce, createObjectURL, revokeObjectURL } = createHarness(api);
    ...
    expect(await blob.text()).toBe(row);
    const objectUrl = createObjectURL.mock.results[0].value;
    expect(clickedAnchor?.getAttribute("href")).toBe(objectUrl);
    ...
    expect(announce).toHaveBeenCalledWith("审计导出已完成（1 行）。", { kind: "success" });
    expect(button.disabled).toBe(false);
    expect(button.textContent).toContain("导出完成");
    expect(page.element.textContent).not.toContain("persisted row");
  });

  it("supports an empty export and strips its zero-row marker", async () => {
    const api = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(exportResponse(completionMarker(0))) });
    const { page, dom, createObjectURL } = createHarness(api);
  ...
  it.each([
    ["missing marker", JSON.stringify({ trace_ref: TRACE_REF }) + "\n"],
    ["duplicate marker", `${completionMarker(0)}${completionMarker(0)}`],
    ["marker before a row", `${completionMarker(0)}${JSON.stringify({ trace_ref: TRACE_REF })}\n`],
    ["rows mismatch", `${JSON.stringify({ trace_ref: TRACE_REF })}\n${completionMarker(2)}`],
    ["marker has an extra top-level field", `${completionMarker(0, { topExtra: { extra: true } })}`],
    ["marker has an extra nested field", `${completionMarker(0, { markerExtra: { extra: true } })}`],
    ["invalid JSON", "not-json\n"],
  ])("fails closed for %s without creating a download", async (_label, body) => {
    const api = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(exportResponse(body)) });
    const { page, dom, alert, createObjectURL } = createHarness(api);
    const click = vi.spyOn(dom.window.HTMLAnchorElement.prototype, "click").mockImplementation(() => {});
    await page.activate({ view: "audit" }, new dom.window.AbortController().signal);
    const button = page.element.querySelector('[data-audit-action="export"]');
    button.click();
    ...
    expect(button.disabled).toBe(false);
    expect(button.textContent).toBe("重试导出");
  });

  it("rejects an oversized line and the 32 MiB ordinary-row budget", async () => {
    const oversizedLine = `${JSON.stringify({ text: "x".repeat(1_048_577) })}\n${completionMarker(1)}`;
    const api = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(exportResponse(oversizedLine)) });
    const first = createHarness(api);
    vi.spyOn(first.dom.window.HTMLAnchorElement.prototype, "click").mockImplementation(() => {});
    await first.page.activate({ view: "audit" }, new first.dom.window.AbortController().signal);
    first.page.element.querySelector('[data-audit-action="export"]').click();
    await vi.waitFor(() => expect(first.alert).toHaveBeenCalled());
    expect(first.createObjectURL).not.toHaveBeenCalled();

    const row = `${JSON.stringify({ text: "x".repeat(1_000_000) })}\n`;
    const manyRows = Array.from({ length: 34 }, () => row).join("") + completionMarker(34);
    const secondApi = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(exportResponse(manyRows)) });
    const second = createHarness(secondApi);
    vi.spyOn(second.dom.window.HTMLAnchorElement.prototype, "click").mockImplementation(() => {});
    await second.page.activate({ view: "audit" }, new second.dom.window.AbortController().signal);
    second.page.element.querySelector('[data-audit-action="export"]').click();
    await vi.waitFor(() => expect(second.alert).toHaveBeenCalled(), { timeout: 10_000 });
    expect(second.createObjectURL).not.toHaveBeenCalled();
  }, 30_000);

  it("handles a reader rejection without saving partial output", async () => {
    const row = `${JSON.stringify({ trace_ref: TRACE_REF })}\n`;
    ...
    const api = userPageApi({ openTraceExport: vi.fn().mockResolvedValue(exportResponse("", { stream })) });
    const { page, dom, alert, createObjectURL } = createHarness(api);
    vi.spyOn(dom.window.HTMLAnchorElement.prototype, "click").mockImplementation(() => {});
    await page.activate({ view: "audit" }, new dom.window.AbortController().signal);
    page.element.querySelector('[data-audit-action="export"]').click();
  ...
  it.each([403, 500])("keeps a %i export failure visible and retryable", async (status) => {
    const api = userPageApi({ openTraceExport: vi.fn().mockRejectedValue(new AdminApiError("export failed", { status })) });
    const { page, dom, alert, createObjectURL } = createHarness(api);
    vi.spyOn(dom.window.HTMLAnchorElement.prototype, "click").mockImplementation(() => {});
    await page.activate({ view: "audit" }, new dom.window.AbortController().signal);
    const button = page.element.querySelector('[data-audit-action="export"]');
    button.click();
    await vi.waitFor(() => expect(alert).toHaveBeenCalledWith("export failed", { title: "导出失败" }));
    expect(createObjectURL).not.toHaveBeenCalled();
    expect(button.disabled).toBe(false);
  });

  it("cancels an in-flight export, prevents a second request, and restores retry", async () => {
    let rejectRequest;
    const openTraceExport = vi.fn().mockImplementation((_query, { signal }) => new Promise((resolve, reject) => {
      rejectRequest = reject;
      signal.addEventListener("abort", () => reject(new DOMException("cancelled", "AbortError")), { once: true });
    }));
    const api = userPageApi({ openTraceExport });
    const { page, dom, announce, createObjectURL } = createHarness(api);
    await page.activate({ view: "audit" }, new dom.window.AbortController().signal);
    const button = page.element.querySelector('[data-audit-action="export"]');
    button.click();
    await vi.waitFor(() => expect(openTraceExport).toHaveBeenCalledOnce());
    button.click();
    page.element.querySelector('[data-audit-action="cancel-export"]').click();
    await vi.waitFor(() => expect(button.textContent).toBe("导出已取消，再次导出"));
    expect(rejectRequest).toBeTypeOf("function");
    expect(openTraceExport).toHaveBeenCalledOnce();
    expect(createObjectURL).not.toHaveBeenCalled();
    expect(announce).toHaveBeenCalledWith("审计导出已取消。");
  });

  it("aborts an in-flight export during route deactivation", async () => {
    let signal;
    const openTraceExport = vi.fn().mockImplementation((_query, options) => {
