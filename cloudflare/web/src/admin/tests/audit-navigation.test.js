...
describe("audit hierarchy and route state", () => {
  ...
  it("drills through the opaque trace_ref and stops explicitly for V1", async () => {
    ...
    expect(page.element.textContent).toContain("旧版记录展示客户端随使用日志上传的过程日志");
  ...
  it("rejects inconsistent legacy and V2 run-detail shapes", async () => {
    const legacy = run({
      source_schema: 1,
      trace_ref: "v1:45",
      run_id: null,
      legacy_id: 45,
      title: "Inconsistent legacy detail",
      outcome: "unknown",
    });
    const v2 = run({ title: "V2 run" });
    const inconsistent = [
      {
        source_schema: 1,
        ...
        run: legacy,
        events: [],
        details: [{ timestamp_utc: 1, level: "Info", message: "声称有步骤数据但携带不可用原因" }],
      },
      {
        source_schema: 1,
        detail_available: false,
      ...
      },
      {
        ...
        run: v2,
        events: [],
        details: [{ timestamp_utc: 1, level: "Info", message: "V2 不携带旧版 details" }],
      },
    ];
    for (const payload of inconsistent) {
      const api = { getTraceRun: vi.fn().mockResolvedValue(payload), getTraceEvent: vi.fn() };
      const { page } = createHarness(api);
      const route = { view: "audit", level: "run", userId: "7", runId: payload.run.trace_ref };
      await page.activate(route);
      expect(page.element.textContent).toContain("运行详情响应格式无效", payload.run.title);
      expect(api.getTraceEvent).not.toHaveBeenCalled();
    }
  });

  it("renders uploaded legacy step details instead of the no-step-data placeholder", async () => {
    const legacy = run({
      source_schema: 1,
      trace_ref: "v1:43",
      run_id: null,
      legacy_id: 43,
      title: "Legacy flash with steps",
      outcome: "success",
      trace_complete: false,
      trace_loss_reason: "legacy_client_no_step_data",
    });
    const api = {
      getTraceRun: vi.fn().mockResolvedValue({
        ...
        details: [
          ...
          { timestamp_utc: 1_787_500_030, level: "Success", message: "Legacy flash完成。" },
          { timestamp_utc: 1_787_500_040, level: "Diagnostic", message: "非闭集级别按原文展示" },
        ],
      }),
      getTraceEvent: vi.fn(),
      getTraceOutput: vi.fn(),
    ...
    await page.activate({ view: "audit", level: "run", userId: "7", runId: "v1:43" });

    expect(page.element.textContent).not.toContain("旧客户端未上传步骤数据");
    expect(page.element.textContent).toContain("旧版步骤数据");
    expect(page.element.textContent).toContain("旧版记录展示客户端随使用日志上传的过程日志");
    expect(page.element.textContent).toContain("正在下载固件包");
    expect(page.element.textContent).toContain("Legacy flash完成。");
    expect(page.element.querySelectorAll("[data-legacy-detail-index]")).toHaveLength(3);
    const badges = [...page.element.querySelectorAll("[data-legacy-detail-index]")]
      .map((item) => item.closest("li")?.querySelector("[data-status]")?.textContent);
    expect(badges).toEqual(["INFO", "SUCCESS", "DIAGNOSTIC"]);
    const badgeClasses = [...page.element.querySelectorAll("[data-legacy-detail-index]")]
      .map((item) => item.closest("li")?.querySelector("[data-status]")?.className);
    expect(badgeClasses).toEqual([
      "audit-status audit-status-info",
  ...
  it("keeps the placeholder only when uploaded legacy details are empty or invalid", async () => {
    const legacy = run({
      source_schema: 1,
      trace_ref: "v1:44",
      run_id: null,
      legacy_id: 44,
      title: "Malformed details row",
      outcome: "failed",
      trace_complete: false,
      trace_loss_reason: "legacy_client_no_step_data",
    });
    const api = {
      getTraceRun: vi.fn().mockResolvedValue({
        ...
        run: legacy,
        events: [],
        details: [{ timestamp_utc: -3, level: "Info", message: "无效时间戳" }, null, { timestamp_utc: 5, level: "Warning", message: "" }],
      }),
      getTraceEvent: vi.fn(),
    };
    const { page } = createHarness(api);

    await page.activate({ view: "audit", level: "run", userId: "7", runId: "v1:44" });

    expect(page.element.textContent).toContain("旧客户端未上传步骤数据");
    expect(page.element.querySelector("[data-legacy-detail-index]")).toBeNull();
    expect(api.getTraceEvent).not.toHaveBeenCalled();
  });
