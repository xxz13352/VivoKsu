...
describe("operational workspace contracts", () => {
  ...
  it("does not announce a create success when the token or id is malformed and refreshes authority once", async () => {
    const { window } = createDom();
    const users = vi.fn()
      .mockResolvedValueOnce({ users: [{ id: 7, username: "alice", name: "Alice", enabled: 1, banned: 0 }] })
      .mockResolvedValueOnce({ users: [{ id: 8, username: "bob", name: "Bob", enabled: 1, banned: 0 }] });
    const createUser = vi.fn().mockResolvedValue({ ok: true, id: 8 });
    const ctx = context({ document: window.document, api: { getUsers: users, createUser } });
    const page = createUsersPage(ctx);
    await page.activate({ view: "users" }, new AbortController().signal);

    page.element.querySelector('[name="username"]').value = "bob";
    page.element.querySelector('[name="password"]').value = "password-123";
    page.element.querySelector('[data-form="create-user"]').dispatchEvent(
      new window.Event("submit", { bubbles: true, cancelable: true }),
    );

    await vi.waitFor(() => expect(createUser).toHaveBeenCalledOnce());
    await vi.waitFor(() => expect(users).toHaveBeenCalledTimes(2));
    await vi.waitFor(() => expect(ctx.alert).toHaveBeenCalledWith(
      "创建结果待确认：未收到有效的一次性令牌。请刷新确认后手动轮换令牌。",
      { title: "创建结果待确认" },
    ));
    expect(ctx.announce).not.toHaveBeenCalledWith("用户已创建；一次性令牌仅显示一次。");
    expect(page.element.querySelector('[data-token-recovery="create"]')).not.toBeNull();
    expect(page.element.querySelector('[data-action="copy-token"]')).toBeNull();
  });

  it("clears a stale token and closes malformed rotation confirmation after one authority refresh", async () => {
    const { window } = createDom();
    const confirmations = [];
    const users = vi.fn()
      .mockResolvedValueOnce({ users: [{ id: 7, username: "alice", name: "Alice", enabled: 1, banned: 0 }] })
      .mockResolvedValueOnce({ users: [{ id: 7, username: "alice", name: "Alice", enabled: 1, banned: 0 }] });
    const rotateUserToken = vi.fn().mockResolvedValue({ ok: true, token: "   " });
    const ctx = context({
      document: window.document,
      confirm: vi.fn((options) => confirmations.push(options)),
      api: { getUsers: users, rotateUserToken },
    });
    const page = createUsersPage(ctx);
    await page.activate({ view: "users" }, new AbortController().signal);
    ...
    expect(rotateUserToken).toHaveBeenCalledOnce();
    expect(users).toHaveBeenCalledTimes(2);
    expect(ctx.announce).not.toHaveBeenCalledWith("令牌已轮换，请立即保存。");
    expect(ctx.alert).toHaveBeenCalledWith(
      "轮换结果待确认：未收到有效的一次性令牌。请在用户行手动再次轮换。",
      { title: "轮换结果待确认" },
    );
    expect(page.element.querySelector('[data-token-recovery="rotate"]')).not.toBeNull();
    expect(page.element.querySelector('[data-action="copy-token"]')).toBeNull();
    expect(page.element.querySelector('[data-action="rotate-token"]').disabled).toBe(false);
  });
