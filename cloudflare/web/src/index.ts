...
import adminFormatTimeJs from "./admin/format-time.js";
...
const STATIC_ASSETS: ReadonlyMap<string, StaticAsset> = new Map([
  ...
  ["/admin/format-time.js", { body: adminFormatTimeJs, contentType: "text/javascript; charset=utf-8" }],
...
async function handleApi(request: Request, url: URL, env: Env): Promise<Response> {
  ...
  // The reliable streaming export mutates the audit log even though it is a
  // GET, so its opt-in protocol still requires the browser-only CSRF signal.
  if (
    method === "GET"
    && path === "/api/usage-logs/v2/export"
    && isReliableExportRequest(request)
  ...
  const appVersionMatch = path.match(/^\/api\/app-versions\/([^/]+)$/);
  if (appVersionMatch && method === "PUT") return updateAppVersion(request, appVersionMatch[1], env);
  if (appVersionMatch && method === "DELETE") return deleteAppVersion(appVersionMatch[1], env);

  // 用户管理
  if (path === "/api/users" && method === "GET") return listUsers(env);
  if (path === "/api/users" && method === "POST") return addUser(request, env);
  const userMatch = path.match(/^\/api\/users\/([^/]+)$/);
  if (userMatch && method === "PUT") return updateUser(request, userMatch[1], env);
  if (userMatch && method === "DELETE") return deleteUser(userMatch[1], env);
  const rotateTokenMatch = path.match(/^\/api\/users\/([^/]+)\/rotate-token$/);
  if (rotateTokenMatch && method === "POST") return rotateUserToken(rotateTokenMatch[1], env);
...
async function addAppVersion(request: Request, env: Env): Promise<Response> {
  const body = await readJsonObject(request);
  if (!body || typeof body.version !== "string") return json({ error: "版本参数无效。" }, 400);
  const version = body.version.trim();
  if (!version) return json({ error: "缺少版本号。" }, 400);
  if (hasOwn(body, "min_version") && typeof body.min_version !== "string") {
    return json({ error: "最低版本参数无效。" }, 400);
  }
  if (hasOwn(body, "download_url") && typeof body.download_url !== "string") {
    return json({ error: "下载地址参数无效。" }, 400);
  }
  if (hasOwn(body, "note") && typeof body.note !== "string") {
    return json({ error: "备注参数无效。" }, 400);
  }
  const minVersion = typeof body.min_version === "string" ? body.min_version.trim() || "0.0.0" : "0.0.0";
  const downloadUrl = typeof body.download_url === "string" ? body.download_url.trim() : "";
  const note = typeof body.note === "string" ? body.note.trim() : "";

  const result = await env.DB.prepare(
    `INSERT INTO app_versions (version, min_version, download_url, note)
     VALUES (?, ?, ?, ?)
     ON CONFLICT(version) DO NOTHING`,
  )
    .bind(version, minVersion, downloadUrl, note)
    .run();
  if (changedRows(result) === 0) return json({ error: "该版本号已存在。" }, 409);
  return json({ ok: true }, 201);
}

async function updateAppVersion(request: Request, rawId: string, env: Env): Promise<Response> {
  const id = parsePositiveIntegerId(rawId);
  if (id === null) return json({ error: "无效 id。" }, 400);
  const body = await readJsonObject(request);
  if (!body) return json({ error: "版本更新参数无效。" }, 400);

  // Column names are a closed code allowlist; only caller-provided values are bound.
  const assignments: string[] = [];
  const values: unknown[] = [];
  if (hasOwn(body, "enabled")) {
    if (typeof body.enabled !== "boolean") return json({ error: "enabled 必须是布尔值。" }, 400);
    assignments.push("enabled = ?");
    values.push(body.enabled ? 1 : 0);
  }
  if (hasOwn(body, "min_version")) {
    if (typeof body.min_version !== "string" || !body.min_version.trim()) {
      return json({ error: "min_version 不能为空。" }, 400);
    }
    assignments.push("min_version = ?");
    values.push(body.min_version.trim());
  }
  if (hasOwn(body, "download_url")) {
    if (typeof body.download_url !== "string") return json({ error: "download_url 必须是字符串。" }, 400);
    assignments.push("download_url = ?");
    values.push(body.download_url.trim());
  }
  if (hasOwn(body, "note")) {
    if (typeof body.note !== "string") return json({ error: "note 必须是字符串。" }, 400);
    assignments.push("note = ?");
    values.push(body.note.trim());
  }
  if (assignments.length === 0) return json({ error: "没有可更新的版本字段。" }, 400);

  const result = await env.DB.prepare(`UPDATE app_versions SET ${assignments.join(", ")} WHERE id = ?`)
    .bind(...values, id)
    .run();
  if (changedRows(result) === 0) return json({ error: "版本不存在。" }, 404);
  return json({ ok: true }, 200);
}

async function deleteAppVersion(rawId: string, env: Env): Promise<Response> {
  const id = parsePositiveIntegerId(rawId);
  if (id === null) return json({ error: "无效 id。" }, 400);
  const result = await env.DB.prepare("DELETE FROM app_versions WHERE id = ?").bind(id).run();
  if (changedRows(result) === 0) return json({ error: "版本不存在。" }, 404);
  return json({ ok: true }, 200);
}
...
async function addUser(request: Request, env: Env): Promise<Response> {
  const body = await readJsonObject(request);
  if (!body || typeof body.username !== "string") return json({ error: "用户参数无效。" }, 400);
  const username = body.username.trim();
  if (!username) return json({ error: "缺少登录账号(username)。" }, 400);
  if (hasOwn(body, "name") && typeof body.name !== "string") return json({ error: "名称参数无效。" }, 400);
  if (typeof body.password !== "string") return json({ error: "密码参数无效。" }, 400);
  if (hasOwn(body, "note") && typeof body.note !== "string") return json({ error: "备注参数无效。" }, 400);
  ...
  const res = await env.DB
    .prepare(
      `INSERT INTO api_users (username, name, token, password, salt, note)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(username) DO NOTHING`,
    )
    .bind(username, name, token, passwordHash, salt, note)
    .run();
  if (changedRows(res) === 0) return json({ error: "登录账号已存在。" }, 409);
  const id = Number(res.meta.last_row_id);
  return json({ ok: true, id, username, name, token }, 201); // token 只在创建时显示一次
}

async function updateUser(request: Request, rawId: string, env: Env): Promise<Response> {
  const id = parsePositiveIntegerId(rawId);
  if (id === null) return json({ error: "无效 id。" }, 400);
  const body = await readJsonObject(request);
  if (!body) return json({ error: "用户更新参数无效。" }, 400);
  ...
  if (hasOwn(body, "banned")) {
    if (typeof body.banned !== "boolean") return json({ error: "banned 必须是布尔值。" }, 400);
    assignments.push("banned = ?");
  ...
  if (hasOwn(body, "newPassword")) {
    if (typeof body.newPassword !== "string" || body.newPassword.length < 6) {
      return json({ error: "新密码至少 6 位。" }, 400);
    }
    const salt = randomHex(16);
    const passwordHash = await pbkdf2(body.newPassword, salt);
    assignments.push("salt = ?", "password = ?");
    values.push(salt, passwordHash);
  }
  if (assignments.length === 0) return json({ error: "没有可更新的用户字段。" }, 400);

  const result = await env.DB.prepare(`UPDATE api_users SET ${assignments.join(", ")} WHERE id = ?`)
    .bind(...values, id)
    .run();
  if (changedRows(result) === 0) return json({ error: "用户不存在。" }, 404);
  return json({ ok: true }, 200);
}

async function deleteUser(rawId: string, env: Env): Promise<Response> {
  const id = parsePositiveIntegerId(rawId);
  if (id === null) return json({ error: "无效 id。" }, 400);
  const result = await env.DB.prepare("DELETE FROM api_users WHERE id = ?").bind(id).run();
  if (changedRows(result) === 0) return json({ error: "用户不存在。" }, 404);
  return json({ ok: true }, 200);
}

async function rotateUserToken(rawId: string, env: Env): Promise<Response> {
  const id = parsePositiveIntegerId(rawId);
  if (id === null) return json({ error: "无效 id。" }, 400);
  const token = randomHex(32);
  const result = await env.DB.prepare("UPDATE api_users SET token = ? WHERE id = ?").bind(token, id).run();
  if (changedRows(result) === 0) return json({ error: "用户不存在。" }, 404);
