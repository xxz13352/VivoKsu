...
/**
 * Detect the opt-in export stream protocol.  The legacy GET response remains
 * byte-compatible until callers explicitly negotiate this profile.
 */
export function isReliableExportRequest(request: Request): boolean {
  const accept = request.headers.get("Accept") ?? "";
  return accept.split(",").some((entry) => {
    const parts = entry.split(";").map((part) => part.trim().toLowerCase());
    return parts[0] === "application/x-ndjson"
      && parts.slice(1).some((part) => part === 'profile="nwflash-export-v1"');
  });
}
...
export async function exportTracesV2(
  ...
  const reliable = isReliableExportRequest(request);
  if (reliable && request.headers.get("X-Requested-With") !== "XMLHttpRequest") {
  ...
  return executeQuery(request, async () => {
    ...
    return new Response(createExportStream(env, exportFilters, firstBatch, reliable), {
      status: 200,
      headers: {
        ...RESPONSE_HEADERS,
        "Content-Type": "application/x-ndjson; charset=utf-8",
        "Content-Disposition": `attachment; filename="nwflash-traces-${Date.now()}.ndjson"`,
        ...(reliable ? { "X-Nwflash-Export-Version": EXPORT_PROTOCOL_VERSION } : {}),
      },
    });
  });
}
...
function createExportStream(
  ...
  function enqueueCompletion(controller: ReadableStreamDefaultController<Uint8Array>): void {
    if (!reliable || markerSent) return;
    const marker = JSON.stringify({
      _nwflash_export: { version: Number(EXPORT_PROTOCOL_VERSION), type: "complete", rows: emittedRows },
    });
...
async function loadTraceDetail(env: Env, parsed: ParsedTraceRef): Promise<TraceRunDetailV2> {
  const runRow = await findCombinedRun(env, parsed.traceRef);
  if (parsed.schema === 1) {
    // 旧版记录的过程日志随 V1 使用日志一并落库(details_json);读出来作为步骤数据展示,
    // 只有确实未上传过的历史客户端行才保留"未上传步骤数据"占位。
    const legacy = await env.DB.prepare(
      "SELECT details_json FROM usage_logs WHERE id = ? AND source_schema = 1",
    ).bind(parsed.id).first<{ details_json: string }>();
    const details = parseLegacyDetails(legacy?.details_json);
    return {
      source_schema: 1,
      detail_available: details.length > 0,
      detail_unavailable_reason: details.length > 0 ? null : "legacy_client_no_step_data",
      run: mapRun(runRow),
      events: [],
      details,
    };
...
function parseLegacyDetails(raw: string | null | undefined): LegacyTraceDetailLineV2[] {
  ...
  const lines: LegacyTraceDetailLineV2[] = [];
  for (const item of parsed) {
    if (!item || typeof item !== "object") continue;
    const record = item as Record<string, unknown>;
    const timestamp = Number(record.timestamp_utc);
    const message = typeof record.message === "string" ? record.message : "";
    if (message.length === 0 || !Number.isFinite(timestamp)) continue;
    lines.push({
      timestamp_utc: Math.trunc(timestamp),
      level: typeof record.level === "string" && record.level.length > 0 ? record.level : "Info",
      message,
    });
  }
  return lines;
