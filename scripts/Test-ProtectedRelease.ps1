    ...
    [ordered]@{
        ...
        markers = @(
            foreach ($marker in Get-NwflashProtectedMarkers) {
                [ordered]@{ symbol = $marker.symbol; name = $marker.name; mode = $marker.mode; compiled = $true }
            }
        )
...
Assert-Condition ($protectedMarkers.Count -eq 8) 'Protected release contract must contain exactly eight stable leaf markers.'
...
'@)
    ...
    Assert-Condition ($layoutFixtureResult.verified -and $layoutFixtureResult.marker_count -eq 8) 'Eight-leaf physical marker fixture did not pass.'
    ...
    Assert-Condition (@($prepared.markers).Count -eq 8) 'Prepared evidence did not bind the exact eight-leaf marker set.'
