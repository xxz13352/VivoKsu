...
function Get-NwflashProtectedMarkers {
    @(
        ...
        [pscustomobject]@{ symbol = 'nwflash_protection_requires_protected_recheck'; name = 'NWFlash.OperationAdmission'; mode = 'Ultra' }
        [pscustomobject]@{ symbol = 'nwflash_protection_terminate_process'; name = 'NWFlash.TerminalExit'; mode = 'Ultra' }
    )
}
...
function Get-PeCodeViewIdentity {
    ...
    try {
        ...
        for ($entry = 0; $entry -lt [Math]::Floor($debugSize / 28); $entry++) {
            ...
            # RSDS header (sig 4 + GUID 16 + age 4) 之后是 NUL 结尾的 PDB 路径。
            # dumpbin /DISASM 与 VMProtect 的符号解析都按这个文件名在 EXE 同目录
            # 查找 PDB；rustc 把 crate 名归一为下划线（nwflash_desktop.pdb），
            # 与交接主名（nwflash-desktop.pdb）不同——缺这份副本时反汇编里
            # 函数标签全部不可见，marker 布局校验必然失败。
            $nameBytes = $reader.ReadBytes([Math]::Max(0, $sizeOfData - 24))
            $nul = [Array]::IndexOf($nameBytes, [byte]0)
            if ($nul -ge 0) { $nameBytes = $nameBytes[0..($nul - 1)] }
            $codeviewPdb = [Text.Encoding]::ASCII.GetString($nameBytes)
            return [pscustomobject]@{
                machine = ('0x{0:X4}' -f $machine)
                guid = ([Guid]::new($guidBytes)).ToString('D').ToUpperInvariant()
                age = [uint32]$age
                pdb_leaf = [IO.Path]::GetFileName($codeviewPdb)
            }
        }
...
function Invoke-PrepareManualHandoffCore {
    ...
    # EXE 的 CodeView 记录按原始文件名（下划线归一，如 nwflash_desktop.pdb）
    # 查找 PDB；再落一份同内容副本供 dumpbin/VMP 符号解析命中。accept 链只
    # 按 prepared.json 的三个主名路径校验，此副本不影响证据链。
    $codeviewIdentity = Get-PeCodeViewIdentity $copiedExe
    $codeviewPdbLeaf = [string]$codeviewIdentity.pdb_leaf
    if ([string]::IsNullOrWhiteSpace($codeviewPdbLeaf)) { throw 'PE CodeView record has no PDB leaf name.' }
    $codeviewPdbCopy = Join-Path $inputDirectory $codeviewPdbLeaf
    if (-not $codeviewPdbCopy.Equals($copiedPdb, [StringComparison]::OrdinalIgnoreCase) -and -not (Test-Path -LiteralPath $codeviewPdbCopy)) {
        & $Operations.CopyFile $copiedPdb $codeviewPdbCopy
    }
...
function Assert-MarkerReview {
    ...
    # Marker regions are keyed by symbol, not by region name: two leaves may
    # legitimately share one VMProtect region name (admit_local_operation and
    # requires_protected_recheck both compile into NWFlash.OperationAdmission).
    $seen = @{}
    foreach ($marker in $actualMarkers) {
        if ($seen.ContainsKey([string]$marker.symbol)) { throw "Marker review contains a duplicate: $($marker.symbol)" }
        $seen[[string]$marker.symbol] = $true
        $expected = @($expectedMarkers | Where-Object symbol -eq ([string]$marker.symbol))
        if ($expected.Count -ne 1 -or [string]$marker.mode -ne [string]$expected[0].mode -or -not [bool]$marker.compiled) { throw "Marker review is invalid for $($marker.symbol)." }
    }
}
