...
pub fn parse_fastboot_partition_table(
    ...
    // 一个分区都没解析出来（getvar 输出为空/全部不可解析）：设备不可达或该
    // fastboot 不支持 getvar。绝不能返回一张空表让上层误报“分区表已读取”，
    // 与 C# 参考 FastbootPartitionService 的 anyRead 语义一致。
    if partitions.is_empty() {
        return Err(DomainError::ExternalTool(
            "无法读取分区信息：getvar 全部失败，设备可能已断开或该 fastboot 不支持 getvar。"
                .to_string(),
        ));
    }
