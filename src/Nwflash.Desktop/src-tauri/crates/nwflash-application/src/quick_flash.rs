...
impl QuickFlashService {
    ...
    /// 执行前校验计划序列号与当前设备一致（对齐 C# `PartitionExecutionService.VerifySession`）。
    /// 绝不静默改写计划目标：用户读取 A 机分区表后若设备被换成 B 机，把 A 机的镜像/
    /// 分区映射继续写入 B 机属于变砖级事故，因此序列号不一致必须失败并要求重新读取。
    pub fn verify_execution_plan_device(
        &self,
        plan: &PartitionExecutionPlan,
        current_serial: &str,
    ) -> Result<(), DomainError> {
