...
use nwflash_infrastructure::remote_firmware::{
    extract_zip_members, list_zip_members, probe_remote_kind, verify_remote_firmware_integrity,
    RemoteFirmwareError, RemoteFirmwareIntegrity, RemoteFirmwareKind,
};
...
pub struct RootOtaExtractOptions<'a> {
    ...
    /// 无承诺时跳过；给了就是硬门（HTTP 源上唯一独立于传输层的锚点）。
    pub integrity: RemoteFirmwareIntegrity,
}
...
impl RootOtaService {
        ...
        P: FnMut(f64),
    {
        ...
        // 服务器给出 sha256/sizeBytes 承诺时，先完整读一遍固件包验签。
        // 只提取目标分区的按需读取验不了全包摘要，所以这里独立流式
        // 验证；无承诺时跳过（上游常为空，保持 C# 可用性语义）。
        if options.integrity != RemoteFirmwareIntegrity::default() {
            report_stage("正在校验固件包完整性".to_string());
            verify_remote_firmware_integrity(
                options.url,
                &options.integrity,
                None,
                &mut is_canceled,
            ...
            .map_err(|error| {
                map_remote_firmware_error(error, "固件包完整性校验失败，已拒绝云提取。")
            })?;
        }

        let kind = probe_remote_kind(options.url, None, &mut is_canceled)
            .map_err(|error| map_remote_firmware_error(error, "探测固件格式失败。"))?;
        ...
        .and_then(|images| {
            if images.boot_image.is_none() {
                return Err(RootOtaError::MissingBoot(
                    "该固件不含可修补的 boot / init_boot 分区，请换一个固件或选本地镜像。"
                        .to_string(),
                )
                .into_domain());
            }
        ...
        P: FnMut(f64),
    {
        ...
        if boot_entry.is_none() && vendor_boot_entry.is_none() {
            return Err(RootOtaError::MissingBoot(
                "该固件不含可修补的 boot / vendor_boot 分区。".to_string(),
            )
            .into_domain());
        }
...
fn map_remote_firmware_error(error: RemoteFirmwareError, prefix: &str) -> DomainError {
    match error {
        ...
        RemoteFirmwareError::UnsupportedFormat => {
            RootOtaError::InvalidFormat("不支持的固件格式，无法云提取 ROOT 分区。".to_string())
                .into_domain()
        }
        RemoteFirmwareError::Archive(_) => {
            RootOtaError::InvalidFormat(format!("{prefix}固件压缩包无法读取或已损坏。"))
                .into_domain()
        }
        RemoteFirmwareError::Integrity(_) => {
            RootOtaError::InvalidFormat(format!("{prefix}固件分区完整性校验失败。")).into_domain()
        }
        RemoteFirmwareError::MissingPartition(_) => {
            RootOtaError::MissingBoot(format!("{prefix}固件不含所需启动分区。")).into_domain()
        }
        RemoteFirmwareError::Transport(_) => {
            RootOtaError::Remote(format!("{prefix}无法读取服务器固件，请重新检测后再试。"))
                .into_domain()
        }
        RemoteFirmwareError::InvalidUrl(_) => {
            RootOtaError::InvalidFormat(format!("{prefix}服务器固件地址无效。")).into_domain()
        }
    }
}
