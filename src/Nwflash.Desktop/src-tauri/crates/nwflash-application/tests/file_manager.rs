...
#[test]
fn transactional_builders_keep_explicit_temp_paths_and_quote_remote_promote() {
    let root = temporary_directory("transactional-builders");
    let source = root.join("update package.zip");
    fs::write(&source, b"fixture").expect("source fixture should be written");
    let service = FileManagerService::with_platform_tools("adb.exe", "fastboot.exe");

    let final_remote = service
        .push_destination("RF8", &source, "/sdcard/O'Brien")
        .expect("final remote destination should build");
    assert_eq!(final_remote, "/sdcard/O'Brien/update package.zip");

    let temporary_remote = "/sdcard/O'Brien/.nwflash-upload-test.partial";
    let push = service
        .build_push_command_to_path("RF8", &source, temporary_remote)
        .expect("temporary push command should build");
    assert_eq!(
        push.args,
        vec![
            "-s",
    ...
    let promote = service
        .build_remote_promote_command("RF8", temporary_remote, &final_remote)
        .expect("remote promote command should build");
    let script = promote.args.last().expect("promote script should be last");
    assert!(script.contains("mv -n --"));
    assert!(script.contains("[ ! -f '/sdcard/O'\\''Brien/.nwflash-upload-test.partial' ] || [ -L '/sdcard/O'\\''Brien/.nwflash-upload-test.partial' ]"));
    assert!(script.contains("exit 73"));
    assert!(script.contains("[ -e '/sdcard/O'\\''Brien/update package.zip' ] || [ -L '/sdcard/O'\\''Brien/update package.zip' ]"));
    assert!(script.contains("[ ! -f '/sdcard/O'\\''Brien/update package.zip' ] || [ -L '/sdcard/O'\\''Brien/update package.zip' ]"));
    assert!(script.contains("[ -L '/sdcard/O'\\''Brien/.nwflash-upload-test.partial' ]"));
    assert!(script.contains("O'\\''Brien"));
    assert!(!script.contains("mv --"));
    assert!(!script.contains("*"));

    let cleanup = service
        .build_remote_remove_file_command("RF8", temporary_remote)
        .expect("remote cleanup command should build");
    assert_eq!(
        cleanup.args,
        vec![
            "-s",
            "RF8",
            "shell",
            "rm -f -- '/sdcard/O'\\''Brien/.nwflash-upload-test.partial'; status=$?; \
             if [ \"$status\" -ne 0 ] || [ -e '/sdcard/O'\\''Brien/.nwflash-upload-test.partial' ] || [ -L '/sdcard/O'\\''Brien/.nwflash-upload-test.partial' ]; then exit 74; fi; \
             exit 0",
        ]
    );
    fs::remove_dir_all(root).expect("temporary directory should be removed");
...
#[test]
fn transactional_pull_builder_targets_a_sibling_path_without_changing_validation() {
    let root = temporary_directory("transactional-pull");
    let destination = root.join("chosen name.zip");
    let temporary = root.join(".chosen name.zip.nwflash-download-test.partial");
    let service = FileManagerService::with_platform_tools("adb.exe", "fastboot.exe");

    let command = service
        .build_pull_command_to_path("RF8", "/sdcard/Download/update.zip", &temporary)
        .expect("temporary pull command should build");
    assert_eq!(
        command.args,
        vec![
            "-s",
