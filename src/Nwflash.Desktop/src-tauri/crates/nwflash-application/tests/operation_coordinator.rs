...
#[tokio::test]
async fn report_preflight_failure_logs_error_and_broadcasts_failed_snapshot_when_idle() {
    let logger = TestLogger::default();
    let coordinator =
    ...
    coordinator
        .report_preflight_failure("读取分区表", "当前没有已连接的 ADB 设备。")
        .await;

    assert!(logger
        .entries()
        .contains(&"当前没有已连接的 ADB 设备。".to_string()));

    let failed = receiver
        .recv()
        .await
        .expect("failed snapshot should broadcast");
    assert_eq!(failed.kind, OperationKind::Failed);
    assert_eq!(failed.title, "读取分区表");
    assert_eq!(failed.stage, "当前没有已连接的 ADB 设备。");

    let idle = receiver.recv().await.expect("idle snapshot should follow");
    assert_eq!(idle.kind, OperationKind::Idle);
}

#[tokio::test]
async fn report_preflight_failure_while_busy_only_writes_the_log_entry() {
    let logger = TestLogger::default();
    let coordinator =
        OperationCoordinator::new(None, None, None, Some(Arc::new(logger.clone())), None);
    let mut receiver = coordinator.subscribe_state();
    ...
    let operation = tokio::spawn(async move {
        coordinator_for_operation
            ...
            .await
            .expect("occupied operation should succeed after release");
    });
    ...
    while !coordinator.is_busy() {
        assert!(
            ...
            "operation should start within timeout"
        );
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
    // 排空“运行中”快照，后续不得再有任何广播。
    ...
    assert!(logger
        .entries()
        .contains(&"当前没有已连接的 ADB 设备。".to_string()));
    let leaked = tokio::time::timeout(Duration::from_millis(100), receiver.recv()).await;
    assert!(
        leaked.is_err(),
        "no snapshot should be broadcast while an operation is running"
    );
...
#[tokio::test]
async fn failed_operation_logs_a_clipped_detail_line_next_to_the_public_message() {
    let logger = TestLogger::default();
    let coordinator =
        OperationCoordinator::new(None, None, None, Some(Arc::new(logger.clone())), None);
    ...
    let result = coordinator
        .run_async(
            ...
            |_, _| async move {
                Err(DomainError::ExternalTool(
                    "ADB Root 分区表读取失败，退出码 1：error: no devices/emulators found"
                        .to_string(),
                ))
            },
        )
    ...
    assert!(entries
        .iter()
        .any(|entry| entry == "外部工具执行失败，请检查设备连接和所需组件后重试。"));
    assert!(
        entries
            .iter()
            .any(|entry| entry.starts_with("失败详情：") && entry.contains("退出码 1")),
        "detail line should be logged: {entries:?}"
    );
}

#[tokio::test]
async fn operation_failure_detail_is_clipped_to_a_bounded_length() {
    let logger = TestLogger::default();
    let coordinator =
        OperationCoordinator::new(None, None, None, Some(Arc::new(logger.clone())), None);
    let noisy = "x".repeat(999);

    let _ = coordinator
        .run_async(
            OperationKind::Discovering,
            "读取 ADB Root 分区表",
            move |_, _| async move { Err(DomainError::RemoteApi(noisy)) },
        )
        .await;
    ...
    let detail = logger
        .entries()
        .into_iter()
        .find(|entry| entry.starts_with("失败详情："))
        .expect("detail line should be logged");
    assert!(
        detail.chars().count() <= 250,
        "detail should be clipped: {}",
        detail.chars().count()
    );
...
#[tokio::test]
async fn cancelled_operation_does_not_log_a_detail_line() {
    let logger = TestLogger::default();
    let coordinator =
        OperationCoordinator::new(None, None, None, Some(Arc::new(logger.clone())), None);
    ...
    let result = coordinator
        .run_async(
            ...
            "读取 ADB Root 分区表",
            |_, _| async move {
                Err(DomainError::UserCancelled("线刷预检已取消。".to_string()))
            },
        )
        .await;

    assert!(matches!(result, Err(OperationCoordinatorError::Canceled)));
    // 取消是用户主动行为，详情行没有排障价值且会污染日志区。
    assert!(logger
        .entries()
        .iter()
        .all(|entry| !entry.starts_with("失败详情：")));
}

/// 命令级明细必须只进上报服务器的使用日志 `details`，不进本地操作日志区
/// （界面日志保持阶段级文案）。
#[tokio::test]
async fn usage_only_details_reach_the_server_without_touching_the_local_log() {
    let usage = UsageEntryRecorder::default();
    let logger = TestLogger::default();
    let coordinator = OperationCoordinator::new(
        None,
    ...
    coordinator
        .run_async(
            ...
            |context, _| async move {
                ...
                context.report_usage_detail("[cmd] fastboot flash boot boot.img → exit=0 · 1.2s");
                context.report_usage_detail("[cmd] fastboot set_active a → exit=0 · 120ms");
                Ok(())
            },
        )
        .await
    ...
    let entries = usage.logs.lock().unwrap().clone();
    ...
    let details = entries[0]
        ...
        .collect::<Vec<_>>();
    // 服务器侧拿到完整命令流水（阶段文案本来也进 details，这里只关心命令级明细）。
    let commands = details
        .iter()
        .filter(|message| message.starts_with("[cmd] "))
        .cloned()
        .collect::<Vec<_>>();
    assert_eq!(
        commands,
        vec![
            "[cmd] fastboot flash boot boot.img → exit=0 · 1.2s".to_string(),
            "[cmd] fastboot set_active a → exit=0 · 120ms".to_string(),
        ]
    );

    // 本地日志区只有阶段文案：命令明细一条都不能出现。
    let local = logger.entries();
    assert!(local.contains(&"正在刷写".to_string()), "{local:?}");
    assert!(local.iter().all(|line| !line.starts_with("[cmd] ")));
}

/// 同一条命令连续重复上报只保留一条（分桶的连续去重语义）。
#[tokio::test]
async fn repeated_identical_usage_details_are_collapsed() {
    let usage = UsageEntryRecorder::default();
    let coordinator =
        OperationCoordinator::new(None, None, Some(Arc::new(usage.clone())), None, None);
    ...
    coordinator
        .run_async(
            ...
            |context, _| async move {
                for _ in 0..5 {
                    context.report_usage_detail("[cmd] fastboot devices → exit=0 · 30ms");
                }
                Ok(())
            },
        )
    ...
    let entries = usage.logs.lock().unwrap().clone();
    let commands = entries[0]
        .details
        .iter()
        .filter(|detail| detail.message.starts_with("[cmd] "))
        .count();
    assert_eq!(commands, 1, "{:?}", entries[0].details);
}
