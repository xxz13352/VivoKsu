...
#[test]
fn fastboot_partition_table_parser_rejects_an_empty_table() {
    // getvar 全部失败/设备不可达时输出为空或不含任何 partition-size 行，
    // 必须报错而不是返回一张空表冒充分区表已读取。
    let error = parse_fastboot_partition_table("FAST123", "")
        .expect_err("empty getvar output must not parse as an empty table");
    assert!(error.to_string().contains("getvar"));

    let error = parse_fastboot_partition_table("FAST123", "(bootloader) current-slot: a\n")
        .expect_err("output without any partition-size rows must be rejected");
    assert!(error.to_string().contains("getvar"));
}
