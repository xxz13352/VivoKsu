...
#[test]
fn quick_flash_rejects_executing_a_plan_on_a_different_device() {
    let service = make_service();
    ...
    // 跨设备防护（对齐 C# VerifySession）：计划属于 SERIAL-A，而当前在线的是
    // SERIAL-B 时必须失败，绝不静默把 A 机的镜像刷进 B 机。
    let error = service
        .verify_execution_plan_device(&prepared_plan, "SERIAL-B")
        .expect_err("executing a plan on a different device must fail");
    assert!(error.to_string().contains("连接设备已变化"));

    // 同一设备时校验通过，命令仍指向计划设备。
    service
        .verify_execution_plan_device(&prepared_plan, "SERIAL-A")
        .expect("executing on the planned device should pass");
    let commands = service
        .build_commands(&prepared_plan)
        .expect("verified plan should build commands");
    assert_eq!(commands[0].args[0..4], ["-s", "SERIAL-A", "flash", "boot"]);
}

#[test]
fn quick_flash_rejects_plans_without_a_serial() {
    let service = make_service();
    let plan = PartitionExecutionPlan {
        serial: "  ".to_string(),
        transport: PartitionTransportKind::Fastboot,
    ...
    let error = service
        .verify_execution_plan_device(&plan, "SERIAL-A")
        .expect_err("a plan without a serial must be rejected");
    assert!(error.to_string().contains("缺少设备序列号"));
}
