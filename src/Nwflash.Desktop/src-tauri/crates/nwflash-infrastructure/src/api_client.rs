...
impl CloudflareError {
    ...
    /// 在线固件查询（`/api/rom`）专用的失败文案。
    ///
    /// `user_message()` 把所有 5xx 都折叠成“服务暂时不可用，请稍后重试”，
    /// 用户无法分辨是服务器炸了还是根本没有这个固件包。这里按状态码区分：
    /// 404 → 没有该固件包；5xx → 服务器/上游故障；其余 → 透传服务端原因。
    pub fn rom_lookup_message(&self) -> String {
        match self {
            CloudflareError::ApiError {
                status: 404,
                message,
                ...
                if message.is_empty() {
                    "未找到该固件包（该机型/版本可能未上架或不受支持）。".to_string()
                } else {
                    message.to_string()
                }
            }
            CloudflareError::ApiError { status, message } if (500..=599).contains(status) => {
                let message = message.trim();
                if message.is_empty() {
                    format!("服务器故障（HTTP {status}），请稍后重试。")
                } else {
                    format!("服务器故障（HTTP {status}）：{message}")
                }
            }
            CloudflareError::ApiError { status, message } => {
                let message = message.trim();
                if message.is_empty() {
                    self.user_message()
                } else {
                    format!("在线固件查询被拒绝（HTTP {status}）：{message}")
                }
            }
...
/// 上次进程崩溃的补传报告(服务端 POST /api/diagnostics/crash)。
/// `panic_message`/`backtrace` 由调用方在构造前完成价值过滤
/// (哈希/凭据等上传无意义的内容不上传);
/// 客户端镜像服务端的闭集字段与长度上限,拒绝即不上线。
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
...
impl CrashReportRequest {
    pub const MAX_PANIC_MESSAGE_BYTES: usize = 16 * 1024;
    pub const MAX_BACKTRACE_BYTES: usize = 32 * 1024;

    fn validate(&self) -> CloudflareResult<()> {
        if !is_identifier(&self.event_id, 64) {
            return Err(CloudflareError::InvalidInput(
                "crash event id is invalid".to_string(),
            ));
        }
        if !is_client_version(&self.client_version) {
            return Err(CloudflareError::InvalidInput(
                "crash client version is invalid".to_string(),
            ));
        }
        if !is_identifier(&self.build_id, 128) {
            return Err(CloudflareError::InvalidInput(
                "crash build id is invalid".to_string(),
            ));
        }
        if !is_identifier(&self.session_id, 64) {
            return Err(CloudflareError::InvalidInput(
                "crash session id is invalid".to_string(),
            ));
        }
        if self.panic_message.is_empty() || self.panic_message.len() > Self::MAX_PANIC_MESSAGE_BYTES
        {
            return Err(CloudflareError::InvalidInput(
                "crash panic message length is invalid".to_string(),
            ));
        }
        if self.backtrace.len() > Self::MAX_BACKTRACE_BYTES {
            return Err(CloudflareError::InvalidInput(
                "crash backtrace length is invalid".to_string(),
            ));
        }
        if !(1..=9_007_199_254_740_991).contains(&self.occurred_at) {
            return Err(CloudflareError::InvalidInput(
                "crash occurrence time is invalid".to_string(),
            ));
        }
        Ok(())
    }
...
impl CloudflareClient {
    ...
    pub fn new_injected(base_url: impl Into<String>, app_version: impl Into<String>) -> Self {
        Self {
            ...
            http: Client::builder()
                ...
                // Match the pinned production builder, which also bypasses the
                // ambient proxy: a developer machine with HTTP_PROXY set must
                // not silently reroute loopback traffic through that proxy.
                // Without this, a closed-port request comes back as a proxy
                // 502 instead of a transport error.
                .no_proxy()
    ...
    pub async fn resolve_rom(
        ...
        // `/api/rom` 的失败必须能区分“没有这个固件包”(404) 与“服务器/上游故障”
        // (5xx)。通用 `handle_api_error` 会丢弃服务端 `error` 文案，这里改为
        // 显式透传，缺失时才回退到状态码兜底文案。
        ...
        if !status.is_success() {
            let message = extract_error_message(text.as_str())
                .unwrap_or_else(|| fallback_message(status.as_u16()));
    ...
    /// 上传上次进程崩溃的补传报告。匿名(token = None)与登录态均可;
    /// 202(首次)/200(幂等重复)都视为成功。429/4xx/5xx 走统一错误映射。
...
/// 从服务端错误响应体里提取 `error` 文案。
///
/// 只用于短错误消息透传（例如 `/api/rom` 的“未找到该固件包”与上游故障原因）；
/// 解析失败或字段缺失时返回 `None`，由调用方回退到状态码兜底文案。
