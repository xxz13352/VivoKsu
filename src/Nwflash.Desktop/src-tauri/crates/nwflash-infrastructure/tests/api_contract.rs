...
async fn resolve_async_maps_insufficient_credits_status() {
    ...
    // /api/rom 的错误处理优先透传服务端 `error` 文案；body 无 `error`
    // 字段（或解析失败）时回退到状态码兜底文案（extract_error_message
    // → fallback_message，HEAD 上已声明的有意设计）。这里验证 402 的
    // 兜底映射「信用点不足」。
    Mock::given(method("GET"))
        .and(path("/api/rom"))
        .and(header("Authorization", "Bearer tok"))
        .respond_with(ResponseTemplate::new(402).set_body_json(serde_json::json!({
            "detail": "unrelated field"
        })))
        .mount(&mock_server)
        .await;
...
#[tokio::test]
async fn upload_usage_logs_posts_the_batch_with_step_details() {
    let mock_server = MockServer::start().await;
    let base_url = mock_server.uri();
    let api = create_client(&base_url);
    let logs = vec![
        UsageLogEntry {
            ...
            details: vec![
                nwflash_domain::UsageLogDetail {
                    timestamp_utc: 1000,
                    level: nwflash_domain::OperationLogLevel::Info,
                    message: "正在下载固件包".to_string(),
                },
                nwflash_domain::UsageLogDetail {
                    timestamp_utc: 1060,
                    level: nwflash_domain::OperationLogLevel::Success,
                    message: "正在刷写 boot完成。".to_string(),
                },
            ],
        },
        UsageLogEntry {
    ...
    // wiremock 对请求体做逐字段精确匹配:details 的字段名、嵌套结构与 PascalCase
    // 级别枚举必须与这里完全一致才返回 200,证明步骤数据确实进入上传请求体。
    Mock::given(method("POST"))
            ...
            "logs": [
                {
                    ...
                    "duration_ms": 60000,
                    "details": [
                        { "timestamp_utc": 1000, "level": "Info", "message": "正在下载固件包" },
                        { "timestamp_utc": 1060, "level": "Success", "message": "正在刷写 boot完成。" }
                    ]
                },
                {
                    "operation": "Rebooting",
