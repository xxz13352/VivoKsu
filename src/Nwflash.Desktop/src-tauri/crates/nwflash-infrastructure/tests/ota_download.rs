...
#[tokio::test]
async fn truncated_range_response_resumes_from_the_bytes_already_on_disk() {
    // 复现 Vivo 固件 CDN 行为：206 声明完整范围，正文中途断开
    // （reqwest 报 `error decoding response body`）。续传必须让下载仍然成功。
    const TOTAL: usize = 512 * 1024;
    const TRUNCATE_AFTER: usize = 37 * 1024;
    let data = (0..TOTAL)
        .map(|index| (index % 251) as u8)
        .collect::<Vec<u8>>();
    let url = common::spawn_unreliable_range_server(data.clone(), Some(TRUNCATE_AFTER), 0);

    let directory = temporary_directory();
    let destination = directory.join("firmware.zip");
    let downloader =
        OtaDownloader::new(Client::new(), Arc::new(FixedDiskSpace(8 * 1024 * 1024)), 20);

    let downloaded = downloader
        .download_to_file(&url, &destination, 1, &CancellationToken::new(), None)
        .await
        .expect("truncated Range responses must be resumed instead of failing the download");

    assert_eq!(downloaded, TOTAL as u64);
    assert_eq!(
        fs::read(&destination).expect("resumed artifact should exist"),
        data,
        "resumed download must reproduce the remote bytes exactly"
    );
    fs::remove_dir_all(directory).expect("fixture directory should be removable");
}

#[tokio::test]
async fn exhausted_parallel_ranges_fall_back_to_a_single_connection() {
    // 前 10 个 Range 请求零字节进展：两个并行分片各耗尽 5 次重试后整计划失败，
    // 必须退化为单连接（第 11 个请求起正常），而不是把整个下载判死。
    const TOTAL: usize = 4 * 1024 * 1024;
    const HALF: usize = TOTAL / 2;
    let data = vec![b'z'; TOTAL];
    let url = common::spawn_unreliable_range_server(data.clone(), None, 10);

    let directory = temporary_directory();
    let destination = directory.join("firmware.zip");
    let downloader = OtaDownloader::new(
        Client::new(),
        Arc::new(FixedDiskSpace(64 * 1024 * 1024)),
        21,
    );

    let downloaded = downloader
        .download_to_file(&url, &destination, 2, &CancellationToken::new(), None)
        .await
        .expect("a dead parallel plan must degrade to a single connection");

    assert_eq!(downloaded, TOTAL as u64);
    assert_eq!(
        fs::read(&destination).expect("fallback artifact should exist"),
        data
    );
...
async fn ota_commit_failure_removes_staging_and_preserves_nonempty_destination_directory() {
    ...
    assert!(error.to_string().contains("提交固件下载结果失败"));
    assert_eq!(
