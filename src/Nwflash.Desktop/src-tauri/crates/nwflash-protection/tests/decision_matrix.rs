...
use nwflash_protection::{
    ...
    encoded_selector, requires_protected_recheck, DecisionInput, LeaseBinding, LeaseClaims,
    LeaseKind, OperationDecision, ProtectedOperationKind, ProtectionDecision, ProtectionFailure,
    ProtectionSelector, SignedEnvelope, TokenDigest,
};
use rand_core::OsRng;
...
#[test]
fn protected_recheck_classification_covers_every_domain_kind_and_fails_closed() {
    // 高危表必须与既有 LocalProtectionGate 分类一致:五类写设备动作强制
    // 走受保护租约复检,其余类别不触发本地复检。
    let high_risk = [
    ...
    for kind in high_risk {
        assert!(requires_protected_recheck(kind as u32), "{kind:?}");
    }
    for kind in passthrough {
        assert!(!requires_protected_recheck(kind as u32), "{kind:?}");
    }

    // 未知/越界 wire 索引必须按高危处理(fail-closed),不允许把新类别
    // 默认归入免检通道。
    for unknown in [u32::MAX, 11, 100, 0xdead_beef] {
        assert!(requires_protected_recheck(unknown), "{unknown}");
    }
}

#[test]
fn protected_wire_table_maps_exhaustively_from_every_domain_kind() {
    // 域枚举到保护圈表的映射是穷举的:每个域 kind 都能映射回自身,任何
    // 新增域 kind 未扩展保护圈表时这里会直接编译失败而非静默漏过。
    for (index, expected) in [
        (nwflash_domain::OperationKind::Idle, ProtectedOperationKind::Idle),
        (
            nwflash_domain::OperationKind::Discovering,
            ProtectedOperationKind::Discovering,
        ),
        (
            nwflash_domain::OperationKind::Rebooting,
            ProtectedOperationKind::Rebooting,
        ),
        (
        ...
        (
            nwflash_domain::OperationKind::Transferring,
            ProtectedOperationKind::Transferring,
        ),
        (
        ...
        (
            nwflash_domain::OperationKind::Flashing,
            ProtectedOperationKind::Flashing,
        ),
        (
            nwflash_domain::OperationKind::Mirroring,
            ProtectedOperationKind::Mirroring,
        ),
        (
            nwflash_domain::OperationKind::Completed,
            ProtectedOperationKind::Completed,
        ),
        (
            nwflash_domain::OperationKind::Canceled,
            ProtectedOperationKind::Canceled,
        ),
        (
            nwflash_domain::OperationKind::Failed,
            ProtectedOperationKind::Failed,
        ),
    ] {
        let mapped = ProtectedOperationKind::from_domain(index)
            .unwrap_or_else(|| panic!("{index:?} must map into the protected table"));
        assert_eq!(mapped as u32, expected as u32);
        assert_eq!(
            ProtectedOperationKind::from_wire_index(expected as u32),
            Some(expected),
            "wire round-trip must be stable for {expected:?}"
        );
    }
}
