...
use nwflash_application::{
    result_to_domain_error, FileManagerService, OperationCommandRecorder, OperationContext,
    OperationCoordinator, RootManager, RootPatchPreflightRequest, RootPatchReadiness, RootService,
    SafeFlashBuildOptions, SafeFlashExecutionRequest, SafeFlashExecutionService,
    SafeFlashPartitionSource, SafeFlashPreparedSource,
};
use nwflash_domain::{
...
use nwflash_windows::{
    bundled_platform_tool,
    process::{
        run_command_with_cancel_recorded, ProcessCommand, ProcessCommandRecorder,
...
fn build_vivo_ksu_patch_commands(
    ...
    Ok(vec![
        ...
        // 整段脚本交给设备端 shell（对齐 C# `ShellAsync`）。不能写成
        // `shell -T sh -c <脚本>`：adb 把 argv 用空格拼接后发给设备，脚本里
        // 的 `&&`/`;`/引号会被设备端 shell 拆散，实测直接语法错误退出 1。
        adb(vec![
            "-s".to_string(),
            serial.to_string(),
            "shell".to_string(),
...
async fn read_connected_kernel_release(
    ...
    let output = tokio::task::spawn_blocking(move || {
        let timeout =
            crate::command_timeout::for_command(&command, crate::command_timeout::ROOT_PATCH);
...
async fn execute_root_patch_command(
    ...
    execute_root_patch_command_output(command, cancellation, recorder)
        .await
        .map(|_| ())
}
...
async fn install_root_manager_core(
    ...
    // 逐条 adb 命令只进上报服务器的使用日志 details，本地日志区不变。
    let command_recorder = OperationCommandRecorder::new(context.clone());
    context.report_stage(format!("正在准备 {manager_label} 管理器"));
    let resources = crate::commands::resources::root_resource_service_with_downloader(app_root);
    ...
    let package_state =
        execute_root_manager_command(verification, cancellation.clone(), command_recorder.clone())
            .await?;
    ...
    let launch =
        build_manager_launch_command(&serial, &verified.package_name, &verified.activity_name)
            .map_err(DomainError::InvalidInput)?;
    execute_root_manager_command(launch, cancellation, command_recorder).await?;
...
async fn patch_vivo_ksu_core(
    ...
    let kmi = if use_automatic_kmi {
        ...
        let release = read_connected_kernel_release(
            serial.clone(),
            cancellation.clone(),
            command_recorder.clone(),
        )
    ...
    let extract_join = task::spawn_blocking({
    ...
    .await;
    // join 失败（内部任务 panic）与提取失败同样必须清理 staging，
    // 对应 C# finally 里的无条件工作区删除。
    let extract_result = match extract_join {
        Ok(result) => {
            result.map_err(|_| DomainError::InvalidOperation("ROOT 管理器库不可用。".to_string()))
        }
        Err(_) => Err(DomainError::Internal("ROOT 库提取任务已中断。".to_string())),
    };
    ...
    let patch_result = async {
        ...
        for (index, command) in commands.into_iter().enumerate() {
            ...
            execute_root_patch_command(command, cancellation.clone(), command_recorder.clone())
                .await?;
    ...
    let cleanup = build_vivo_ksu_patch_cleanup_command(&serial, partition)
        .map_err(DomainError::InvalidInput)?;
    let _ = execute_root_patch_command(
        cleanup,
...
async fn patch_official_vendor_boot_core(
    ...
    let patch_result: Result<FlashImageInfo, DomainError> = async {
        ...
        for (index, command) in setup.into_iter().enumerate() {
            ...
            let output = execute_root_patch_command_output(
                command,
                cancellation.clone(),
                command_recorder.clone(),
            )
        ...
        for directory in resolve_vendor_boot_module_directories(&listing) {
            for file_name in ["modules.load", "modules.load.recovery", "modules.softdep"] {
                ...
                if let Err(error) = update {
                    ...
                    // 版本化模块目录允许缺失该文件（预期容忍），但连接类失败
                    // 也走这条路——留一条 Warning 痕迹，后续 repack 失败时才有
                    // 最准确的出错位置可查，而不是无声吞掉。
                    context.report_warning(format!(
                        "vendor_boot 模块目录 {directory} 的 {file_name} 更新未完成。"
                    ));
                }
            }
        }
        ...
        let repack_pull =
            build_vendor_boot_repack_pull_commands(&serial, &remote_root, &staged_output)
                .map_err(DomainError::InvalidInput)?;
        let repack_output = execute_root_patch_command_output(
            repack_pull[0].clone(),
            cancellation.clone(),
            command_recorder.clone(),
        )
...
/// 把 ROOT 修补镜像导出到桌面的 `VivoKsu_修补镜像` 目录（对应 C#
/// `RootViewModel.ExportToDesktop` / `RootPatchArtifactService.ExportToDesktop`），
/// 用户始终能拿到修补产物文件留档。
#[tauri::command]
pub async fn root_export_patched_artifact(
    state: State<'_, AppState>,
    artifact_id: String,
) -> Result<Vec<nwflash_domain::FlashImageInfo>, String> {
    let lease = state
        .session_capabilities
        .capture()
        .map_err(|_| ROOT_CAPABILITY_UNAVAILABLE.to_string())?;
    ...
    let desktop = desktop_directory().ok_or_else(|| "无法定位桌面目录。".to_string())?;
    let exported = nwflash_infrastructure::RootPatchArtifactService::new()
        .export_to_directory(std::slice::from_ref(&artifact.image), &desktop)
    ...
    fn vivo_ksu_patch_uses_the_fixed_remote_workspace_and_validated_kmi() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "ADB-ROOT-1",
                "shell",
                // 整段脚本作为单参数交给设备端 shell（对齐 C# ShellAsync）；
                // `shell -T sh -c <脚本>` 会被 adb 的 argv 空格拼接拆散。
