...
const ROOT_OTA_CAPABILITY_UNAVAILABLE: &str = "ROOT 云提取会话能力已失效，请重新登录后再试。";
...
struct ResolvedRootOta {
    ...
    pd: String,
    version: String,
    /// 服务器透传的整包完整性承诺（sha256/sizeBytes，上游常为空）。
    /// 云提取时给了就必验；防 HTTP 源被 MITM 换包。
        ...
        self.scope
            .commit(lease, || {
                ...
                let resolved = state
                    ...
                    .ok_or_else(|| "尚未检测到服务器固件，请先检测。".to_string())?;
                Ok(resolved.clone())
            })
            .map_err(|_| ROOT_OTA_CAPABILITY_UNAVAILABLE.to_string())?
    }
...
pub async fn root_ota_check(state: State<'_, AppState>) -> Result<RootOtaCheckDto, String> {
    ...
    let serial = match state.device_runtime.active_adb_serial() {
        ...
        Err(_) => {
            state.operation_log_store.write(
                OperationLogLevel::Warning,
                "ROOT 固件检测已跳过：未检测到 ADB 设备。".to_string(),
                None,
            );
            return Ok(RootOtaCheckDto {
                available: false,
    ...
    let rom = match client.resolve_rom(token.as_str(), &pd, &version).await {
        Ok(rom) => rom,
        Err(error) => {
            // 此前服务器查询失败是静默的，用户点了"检测服务器 OTA"却毫无反馈。
            // 这里补一条 Error 日志，让日志区明确给出失败原因（PD/版本 + 服务端原因）。
            state.operation_log_store.write(
                OperationLogLevel::Error,
                format!(
                    "ROOT 固件检测失败（PD: {pd}，版本: {version}）：{}",
                    nwflash_application::sanitize_operation_detail(&error.user_message())
                ),
                None,
            );
            return Ok(RootOtaCheckDto {
    ...
    if runtime
        .store(
            ...
            ResolvedRootOta {
                ...
                version,
                integrity: RemoteFirmwareIntegrity {
                    sha256_hex: rom.sha256,
                    size_bytes: rom.size_bytes.and_then(|size| u64::try_from(size).ok()),
                },
            },
        )
        .is_err()
