...
impl ProcessTerminator for ProductionProcessTerminator {
    fn terminate(&self, exit_code: i32) {
        // 终结动作收进保护圈叶子:圈外打补丁拔掉 request()/worker 的
        // 牙齿后,每个终局路径仍从这里同步消亡(审查发现的退出单点)。
        nwflash_protection::terminate_protected_process(exit_code);
    }
}
...
impl ExitSupervisorHandle {
    ...
    pub(crate) fn request(&self, request: ExitRequest) -> ExitRequestDisposition {
        ...
        if notify_worker && self.sender.send(()).is_err() {
            // 通道消失：无法保证有序收尾，按篡改终局码失败关闭。
            call_terminator_once(&self.control, &self.terminator, PROTECTED_EXIT_CODE);
...
impl ExitSupervisorWorker {
    pub(crate) async fn run(mut self) {
        ...
        if result.is_err() {
            // Worker panic 走失败关闭路径：无法保证有序收尾，按篡改终局码
            // 立即终结。
            call_terminator_once(&self.control, &self.terminator, PROTECTED_EXIT_CODE);
        }
    }
    ...
    async fn run_inner(&mut self) {
        ...
        // ImmediateTamper（VMP 完整性失败）的「立即退出」不能被卡住的在途
        // 操作无限推迟（审计 A13）：等待在途操作以篡改收尾期限为上限，到点
        // 放弃等待直接进入有界收尾（fastboot/adb 子进程随进程消亡按 handoff
        // §1.3 语义自然跑完）。Delayed 保持原语义——等在途操作自然结束后
        // 优雅收尾。退出码在移动前确定（审计 A27）：Delayed 优雅收尾传 0
        //（vmp.rs 文档「正常收尾传 0」、lib.rs 心跳即杀通道也传 0），仅
        // ImmediateTamper 传 70。
        let tamper_exit = matches!(initial.policy, AcceptedPolicy::Immediate { .. });
        let idle = match initial.policy {
            AcceptedPolicy::Immediate { .. } => tokio::time::timeout(
                TAMPER_CLOSEOUT_DEADLINE,
                self.control.coordinator.wait_until_idle(),
            )
            .await
            .ok(),
            AcceptedPolicy::Delayed { .. } => Some(self.control.coordinator.wait_until_idle().await),
        };
        ...
        // 退出码按策略（审计 A27）：Delayed 是优雅收尾，应传 0（vmp.rs
        // 文档写「正常收尾传 0」，lib.rs 心跳即杀通道也传 0）；仅
        // ImmediateTamper 篡改终局传 70。旧实现所有终局恒传 70，运维
        // 按退出码排障会把正常退出误读为篡改退出。
        let exit_code = if tamper_exit {
            PROTECTED_EXIT_CODE
        } else {
            0
        };
        call_terminator_once(&self.control, &self.terminator, exit_code);
    }
}
...
fn call_terminator_once(
    ...
    if should_terminate {
        terminator.terminate(exit_code);
    }
}
