...
impl DeviceTransport {
    ...
    pub fn build_adb_root_copy_from_device_command(
        ...
        let arguments = vec![
            "exec-out".to_string(),
            "su".to_string(),
            // Never pass `--no-pty` here: it is a private su extension that
            // KernelSU rejects with exit code 255 ("Unrecognized option:
            // 'no-pty'"). `exec-out` already streams raw bytes without a PTY,
            // so binary dumps stay clean (measured CRLF/LF ratio ~0.008 on a
            // 4 MiB boot image, i.e. no LF -> CRLF translation).
            "-c".to_string(),
            "dd".to_string(),
    ...
    pub fn build_adb_root_copy_staged_file_to_device_command(
        ...
        self.platform_tools.adb_command(
            ...
            &[
                "shell".to_string(),
                // `-T` disables PTY allocation on the adb side, which is the
                // portable way to keep output raw; `--no-pty` is not
                // understood by every su implementation (KernelSU exits 255).
                "-T".to_string(),
                "su".to_string(),
                "-c".to_string(),
                shell_quote(&command),
    ...
    pub fn build_adb_root_shell_command(
        ...
        let arguments = vec![
            "shell".to_string(),
            // `-T` (adb) is the portable no-PTY switch. `--no-pty` is a
            // vendor-specific su flag: KernelSU aborts with exit code 255,
            // which used to surface as a generic "external tool failed".
            "-T".to_string(),
            "su".to_string(),
            "-c".to_string(),
            shell_quote(command),
    ...
    /// `adb -s <serial> shell uname -r`：设备概览「内核版本」行的唯一来源。
    /// 只允许这一条固定命令——不接受调用方拼接 shell 文本，避免把设备 shell
    /// 暴露成通用执行入口。
    pub fn build_adb_kernel_version_command(
        &self,
        serial: &str,
    ) -> Result<ProcessCommand, DomainError> {
        validate_serial(serial)?;
...
fn validate_device_path(device_path: &str) -> Result<(), DomainError> {
    ...
    // The leading empty component of an absolute path (`/dev/...` splits to
    // `""`, `"dev"`, ...) is expected and must not reject valid paths.
    if device_path
        .strip_prefix('/')
...
mod tests {
    ...
    #[test]
    fn adb_root_shell_quotes_the_complete_command_as_one_argument() {
        let transport = DeviceTransport::new(PlatformTools::new("adb.exe", "fastboot.exe"));
