...
/// Directory identity for the per-process verification cache: the manifest
/// bytes plus every tracked file's (size, mtime). Any on-disk change — a
/// swapped binary or edited manifest — produces a new key, so a cached pass
/// can never outlive the exact file set it verified.
fn platform_tools_fingerprint(root: &Path) -> Option<u64> {
    use std::hash::{Hash, Hasher};

    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    let manifest = std::fs::read(root.join(PLATFORM_TOOLS_MANIFEST)).ok()?;
    manifest.hash(&mut hasher);
    for (file_name, _) in REQUIRED_PLATFORM_TOOLS {
        let metadata = std::fs::metadata(root.join(file_name)).ok()?;
...
/// Cache of `verify_bundled_platform_tools` results, keyed by the directory
/// fingerprint. `process::validate_command` re-runs this on every adb/fastboot
/// spawn; hashing ~10 MB of tooling per command is measurable I/O during a
/// flash chain that issues a dozen commands, so a clean verification is
/// remembered for the process lifetime and invalidated the moment any tracked
/// file or the manifest changes on disk.
static VERIFIED_BUNDLED_TOOLS: OnceLock<std::sync::Mutex<Option<(u64, bool)>>> = OnceLock::new();

fn remember_verified_bundled_tools(key: u64, ok: bool) {
    let mut cache = VERIFIED_BUNDLED_TOOLS
        .get_or_init(|| std::sync::Mutex::new(None))
        .lock()
        .expect("platform-tools verification cache lock should not be poisoned");
    *cache = Some((key, ok));
}

fn is_verified_bundled_tools_cached(key: u64) -> Option<bool> {
    let cache = VERIFIED_BUNDLED_TOOLS
        .get_or_init(|| std::sync::Mutex::new(None))
        .lock()
        .expect("platform-tools verification cache lock should not be poisoned");
    cache
...
pub fn verify_bundled_platform_tools(root: &Path) -> Result<(), DomainError> {
    if let Some(key) = platform_tools_fingerprint(root) {
        ...
        let result = verify_bundled_platform_tools_uncached(root);
        remember_verified_bundled_tools(key, result.is_ok());
        return result;
    }
    // Fingerprint unreadable (missing manifest/binary): fall through to the
    // uncached path so the original, precise error surfaces.
    verify_bundled_platform_tools_uncached(root)
}

fn verify_bundled_platform_tools_uncached(root: &Path) -> Result<(), DomainError> {
...
mod tests {
    ...
    #[test]
    fn verification_cache_is_invalidated_by_a_changed_file() {
        let nonce = std::process::id();
        let root = std::env::temp_dir().join(format!("nwflash-platform-tools-cache-{nonce}"));
        std::fs::create_dir_all(&root).expect("fixture directory should exist");
        let shipped_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("..")
            .join("..")
            .join("resources")
            .join("platform-tools");

        // Seed the fixture with the real, verifiable package, then prime the
        // cache for its fingerprint. A second identical check must pass without
        // re-hashing (cache hit), while swapping one file must fail closed.
        for (file_name, _) in REQUIRED_PLATFORM_TOOLS {
            std::fs::copy(shipped_root.join(file_name), root.join(file_name))
                .expect("fixture tool should be copied");
        }
        std::fs::copy(
            shipped_root.join(PLATFORM_TOOLS_MANIFEST),
            root.join(PLATFORM_TOOLS_MANIFEST),
        )
        .expect("fixture manifest should be copied");

        verify_bundled_platform_tools(&root).expect("the shipped package should verify");
        verify_bundled_platform_tools(&root)
            .expect("an unchanged fingerprint must reuse the cached pass");

        std::fs::write(root.join("adb.exe"), b"tampered").expect("adb.exe should be replaced");
        let error = verify_bundled_platform_tools(&root)
            .expect_err("a changed fingerprint must re-verify and fail");
        assert!(
            error.to_string().contains("内置 Android platform-tools 完整性校验失败"),
            "tampered package must surface the integrity error, got: {error}"
        );

        // A failed verification is not cached as a pass: restoring the real
        // file makes the same directory pass again instead of staying stuck on
        // the tampered result.
        std::fs::copy(shipped_root.join("adb.exe"), root.join("adb.exe"))
            .expect("adb.exe should be restored");
        verify_bundled_platform_tools(&root).expect("the restored package should verify");
