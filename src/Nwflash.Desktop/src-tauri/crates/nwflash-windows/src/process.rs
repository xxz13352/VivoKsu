...
pub trait CancellableProcessExecutor: Send + Sync {
    ...
    /// Runs one process with an optional wall-clock deadline. Existing
    /// executors retain their old behavior through the default implementation;
    /// transactional file transfers use the system override below so timeout
    /// remains part of the cleanup contract without breaking older test doubles.
    fn run_with_timeout(
        &self,
        spec: ProcessCommand,
...
impl CancellableProcessExecutor for SystemCancellableProcessExecutor {
    ...
    fn run_with_timeout(
        ...
        run_command_with_cancel(spec, timeout, should_cancel)
    }
}

/// 一条已执行命令的现场记录。
///
/// 与 [`ProcessStartMetadata`] 同样**刻意不含子进程环境**：环境变量值可能携带
/// 凭据，绝不允许跨出观测边界。`stdout`/`stderr` 是已经按上限截断的字节切片，
/// `*_truncated` 标明是否发生过截断（截断只影响留痕长度，不影响命令结果）。
pub struct ProcessCommandRecord<'a> {
    pub program: &'a str,
    pub args: &'a [String],
    pub exit_code: Option<i32>,
    pub termination: ProcessTermination,
...
impl std::fmt::Debug for ProcessCommandRecord<'_> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ProcessCommandRecord")
            .field("program", &"[STRUCTURED]")
            .field("args", &format_args!("[{} STRUCTURED]", self.args.len()))
            .field("exit_code", &self.exit_code)
...
/// 逐命令执行记录的同步接收端。
///
/// 回调运行在进程观测线程上：实现方**必须保持同步、有界、非阻塞**，不得在其中
/// 做磁盘 I/O、HTTP 或等待锁；需要落盘/上报时应先转成自有结构再异步处理。
pub trait ProcessCommandRecorder: Send + Sync {
    fn record(&self, record: ProcessCommandRecord<'_>);
}

/// 丢弃全部记录的接收端：给「该调用点不需要留痕」的场合一个显式选择，
/// 避免调用方为了满足签名把 `Option` 一路透传。
#[derive(Debug, Clone, Copy, Default)]
pub struct DiscardingCommandRecorder;

impl ProcessCommandRecorder for DiscardingCommandRecorder {
    fn record(&self, _record: ProcessCommandRecord<'_>) {}
}

/// [`run_command_with_cancel`] 的留痕版本：执行语义完全一致，额外把本次执行的
/// argv / 退出码 / 耗时 / 输出交给 `recorder`。
pub fn run_command_with_cancel_recorded<F>(
    spec: ProcessCommand,
    timeout: Option<Duration>,
    should_cancel: F,
...
/// [`run_command_with_file_stdout_and_cancel`] 的留痕版本：stdout 仍然写文件，
/// 观测到的 stdout 分片只用于留痕长度统计（二进制流不进明细）。
pub fn run_command_with_file_stdout_and_cancel_recorded<F>(
    spec: ProcessCommand,
...
where
    F: FnMut() -> bool,
{
    let observer = Arc::new(RecordingProcessObserver::new(
        recorder,
        RecordingProcessExecutor::DEFAULT_OUTPUT_LIMIT_BYTES,
    ));
    let observer_trait: Arc<dyn ProcessOutputObserver> = Arc::clone(&observer) as Arc<_>;
    ...
    observer.record_if_unfinished(outcome.result.as_ref().ok().map(|output| output.exit_code));
...
where
    F: FnMut() -> bool,
{
    let observer = Arc::new(RecordingProcessObserver::new(recorder, output_limit));
    let observer_trait: Arc<dyn ProcessOutputObserver> = Arc::clone(&observer) as Arc<_>;
    let outcome = run_command_with_cancel_observed(spec, timeout, should_cancel, observer_trait);
    // 观测队列溢出/断连时 `Finished` 可能丢失；此时用返回值补一条记录，
    // 避免「命令跑了但服务器上完全没有痕迹」这种最坏情况。
    observer.record_if_unfinished(outcome.result.as_ref().ok().map(|output| output.exit_code));
    outcome.into_legacy_result()
}
...
/// `run_command_with_cancel_observed` 内核、同样的取消与超时行为），只是把每次
/// 执行的 argv / 退出码 / 耗时 / 输出交给 [`ProcessCommandRecorder`]。观测是
/// 纯旁路：记录端出错或丢弃**不会**改变命令结果。
pub struct RecordingProcessExecutor {
    recorder: Arc<dyn ProcessCommandRecorder>,
    output_limit: usize,
}
...
impl RecordingProcessExecutor {
    ...
    pub fn new(recorder: Arc<dyn ProcessCommandRecorder>) -> Self {
        Self {
            recorder,
            output_limit: Self::DEFAULT_OUTPUT_LIMIT_BYTES,
        }
    }
...
impl std::fmt::Debug for RecordingProcessExecutor {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("RecordingProcessExecutor")
            .field("output_limit", &self.output_limit)
            .finish()
    }
}

impl CancellableProcessExecutor for RecordingProcessExecutor {
    fn run(
        &self,
        spec: ProcessCommand,
        should_cancel: &mut dyn FnMut() -> bool,
...
/// 单条命令的观测累加器。与 [`ProcessTraceCollector`] 同契约：**一个实例只服务
/// 一次进程执行**，重复的 `Started` 视为观测丢失。
...
impl RecordingProcessObserver {
    fn new(recorder: Arc<dyn ProcessCommandRecorder>, output_limit: usize) -> Self {
        Self {
            recorder,
            output_limit,
    ...
    /// 收口一条记录并交给记录端。已经收口过则返回 `None`（`Finished` 与兜底
    /// 补记只允许其中之一生效）。
    ///
    /// `finish` 为 `None` 表示观测链没等到 `Finished`，按 `TerminationUnconfirmed`
    /// 留痕；此时若调用方已拿到进程返回值，用 `fallback_exit_code` 补齐退出码。
    fn finish(
        &self,
        finish: Option<ProcessFinishMetadata>,
        fallback_exit_code: Option<i32>,
    ) -> Option<ProcessFinishMetadata> {
        let (metadata, started_at, stdout, stderr, stdout_truncated, stderr_truncated) = {
        ...
        // 记录端在锁外调用：实现方不得持锁重入本观察器。
        let metadata = metadata?;
        let finish = finish.unwrap_or(ProcessFinishMetadata {
        ...
        let duration = started_at
            ...
            .unwrap_or_default();
        self.recorder.record(ProcessCommandRecord {
    ...
    fn record_if_unfinished(&self, fallback_exit_code: Option<i32>) {
        self.finish(None, fallback_exit_code);
    }

    fn push_output(&self, stream: ProcessOutputStream, bytes: &[u8]) {
        let mut state = self.state.lock().unwrap_or_else(|value| value.into_inner());
        if state.recorded {
            return;
        }
        ...
        let (buffer, truncated) = match stream {
            ProcessOutputStream::Stdout => (stdout, stdout_truncated),
            ProcessOutputStream::Stderr => (stderr, stderr_truncated),
        };
        let remaining = self.output_limit.saturating_sub(buffer.len());
        if remaining == 0 {
            *truncated = true;
            return;
        }
...
impl ProcessOutputObserver for RecordingProcessObserver {
    fn observe(&self, observation: ProcessObservation<'_>) -> Result<(), ProcessObserverError> {
        match observation {
            ProcessObservation::Started(metadata) => {
                let mut state = self.state.lock().unwrap_or_else(|value| value.into_inner());
                if state.recorded || state.started.is_some() {
            ...
            ProcessObservation::Output { stream, bytes, .. } => {
                // stdout 直写文件的命令（分区导出）没有可读文本：观测到的分片是
                // 二进制流，收进留痕只会变成乱码，直接跳过。
                if stream == ProcessOutputStream::Stdout {
                    let binary = {
                        let state = self.state.lock().unwrap_or_else(|value| value.into_inner());
                        state.started.as_ref().is_some_and(|started| {
                            started.stdout_mode != ProcessStdoutMode::TextPipe
                        })
                    };
                    if binary {
                        return Ok(());
                ...
                self.push_output(stream, bytes);
                Ok(())
            }
            ProcessObservation::Finished(metadata) => match self.finish(Some(metadata), None) {
                Some(_) => Ok(()),
                None => Err(ProcessObserverError),
            },
        }
    }
}
...
fn production_spawn_site_allowlist() -> std::collections::BTreeMap<String, usize> {
    [
        // mirror.rs: terminate_process_tree 的 taskkill + kill_stale_mirror_process
        // 的孤儿 PID 强杀（N16）。
        ("nwflash-tauri/src/commands/mirror.rs".to_string(), 3),
...
fn unobserved_process_caller_allowlist() -> std::collections::BTreeMap<String, usize> {
    [
        ...
        // 只剩 fastboot 探测（`FastbootProbe` 是测试替身接缝，换成留痕调用
        // 需要同步改所有替身）。刷写/切换槽位/重启/分区表/备份等命令已全部
        // 走留痕通道。
        ("nwflash-tauri/src/commands/quick_flash.rs".to_string(), 1),
        ("nwflash-windows/src/driver.rs".to_string(), 1),
...
mod tests {
    ...
    #[test]
    fn observer_callback_failure_records_one_deterministic_loss() {
        let dispatcher =
            ObservationDispatcher::new(Arc::new(RecordingObserver::failing_first_stdout()));
    ...
    fn observer_failure_reports_loss_without_stopping_pipe_drain() {
        ...
        assert_eq!(output.stdout.len(), expected_len);
        assert!(!outcome.observer_losses.is_empty());
        assert!(outcome.observer_losses.len() <= PROCESS_OBSERVER_MAX_LOSSES);
        assert!(
            outcome.observer_losses.contains(&ProcessObserverLoss {
                callback: ProcessObserverCallback::Output,
                stream: Some(ProcessOutputStream::Stdout),
                sequence: Some(0),
            }),
            "the observer's first stdout rejection must remain observable: {:?}",
            outcome.observer_losses
        );
        for (index, loss) in outcome.observer_losses.iter().enumerate() {
            assert_eq!(loss.callback, ProcessObserverCallback::Output);
            assert_eq!(loss.stream, Some(ProcessOutputStream::Stdout));
            assert!(loss.sequence.is_some());
            assert!(
                !outcome.observer_losses[..index].contains(loss),
                "the same observer loss must not be counted twice: {loss:?}"
            );
        }
    ...
    #[derive(Default)]
    struct RecordingCommandRecorder {
        records: Arc<Mutex<Vec<RecordedCommand>>>,
    }
    ...
    impl ProcessCommandRecorder for RecordingCommandRecorder {
        fn record(&self, record: ProcessCommandRecord<'_>) {
            self.records
                .lock()
                .expect("command record lock should hold")
                .push(RecordedCommand {
                    program: record.program.to_string(),
                    args: record.args.to_vec(),
                    exit_code: record.exit_code,
                    termination: record.termination,
                    stdout: record.stdout.to_vec(),
                    stderr: record.stderr.to_vec(),
                });
        }
    }

    #[test]
    fn recorded_execution_reports_argv_exit_code_and_output() {
        let recorder = Arc::new(RecordingCommandRecorder::default());
        let executor = RecordingProcessExecutor::new(Arc::clone(&recorder) as Arc<_>);
        let output = executor
            .run_with_timeout(
                ProcessCommand::new(
                    "cmd",
                    ["/C".to_string(), "echo".to_string(), "flash-ok".to_string()],
                ),
                None,
                &mut || false,
            )
        ...
        assert_eq!(output.exit_code, 0);
        let records = recorder.records.lock().expect("lock should hold").clone();
        assert_eq!(records.len(), 1, "{records:#?}");
        let record = &records[0];
        assert_eq!(record.program, "cmd");
        assert_eq!(record.args, vec!["/C", "echo", "flash-ok"]);
        assert_eq!(record.exit_code, Some(0));
        assert_eq!(record.termination, ProcessTermination::Completed);
        let stdout = String::from_utf8_lossy(&record.stdout);
        assert!(stdout.contains("flash-ok"), "{stdout:?}");
    }

    #[test]
    fn recorded_execution_reports_one_record_per_command() {
        let recorder = Arc::new(RecordingCommandRecorder::default());
        let executor = RecordingProcessExecutor::new(Arc::clone(&recorder) as Arc<_>);
        for _ in 0..3 {
            executor
                .run_with_timeout(
                    ProcessCommand::new(
                        "cmd",
                        ["/C".to_string(), "exit".to_string(), "3".to_string()],
                    ),
                    None,
                    &mut || false,
                )
        ...
        assert!(records.iter().all(|record| record.exit_code == Some(3)));
    }

    #[test]
    fn discarding_recorder_is_a_no_op() {
        let output = run_command_with_cancel_recorded(
            ProcessCommand::new(
                "cmd",
                ["/C".to_string(), "echo".to_string(), "x".to_string()],
            ),
            None,
            || false,
            Arc::new(DiscardingCommandRecorder),
        )
        .expect("fixture command should complete");
        assert_eq!(output.exit_code, 0);
    }
