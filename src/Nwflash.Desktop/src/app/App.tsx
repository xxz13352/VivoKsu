...
import {
  ...
  type SafeFlashPartitionFailurePayload,
...
import { PartitionFailureDialog } from '../components/PartitionFailureDialog';
...
const hasTauriRuntime = (): boolean => {
  ...
  // 只认注入层 `__TAURI_INTERNALS__`（npm bundle 的 IPC 通道）。
  // `window.__TAURI__` 全局绑定已随 withGlobalTauri=false 移除：拿到
  // WebView JS 执行权不再天然获得一个全局 invoke 入口。
...
export const App: FC = () => {
  ...
  const dismissPartitionFailure = useCallback((resolvedFailure: SafeFlashPartitionFailurePayload) => {
    setPartitionFailure((current) => (current === resolvedFailure ? null : current));
  }, []);

  useEffect(() => {
    if (!isLoggedIn) {
      setPartitionFailure(null);
    }
  }, [isLoggedIn]);
  ...
  // 操作 busy→idle 沿：后端会做补偿刷新，前端也主动重读一次，保证设备概览
  // 在任务结束后一定拿到新状态（后端补偿可能撞上刷新互斥而被跳过）。
  // 初值 true：应用在任务中途启动时，第一条非 busy 快照也要触发一次重读。
  const deviceRefreshPendingRef = useRef(true);
  useEffect(() => {
    const busy = Boolean(operationSnapshot?.isBusy);
    if (deviceRefreshPendingRef.current && !busy) {
      void refreshDevice();
    }
    deviceRefreshPendingRef.current = busy;
  }, [operationSnapshot, refreshDevice]);
  ...
  useEffect(() => {
    ...
    const listenersReady = Promise.all([
      ...
      captureListener(listen<SafeFlashPartitionFailurePayload>(
        IPC_EVENTS.safeFlashPartitionFailure,
        (event) => {
          if (!active) {
            return;
          }
          // 线刷执行中某分区刷写失败：后端已挂起等待决策，弹窗让用户
          // 选择继续刷写剩余分区还是中止本次线刷。
          setPartitionFailure(event.payload);
        },
      )),
    ]);
  ...
  return (
        ...
        modalChildren={readinessDialog === 'resources' ? (
          ...
          <section className="nw-driver-readiness">
            ...
            <div className="nw-driver-dialog-actions">
              ...
              <button type="button" className="nw-dialog-confirm" disabled={readinessBusy} onClick={() => void installDrivers()}>
      ...
      <PageContainer flushTop={pageOwnsHeader}>
        <PageFactory
          ...
          deviceSnapshot={deviceSnapshot}
          operationSnapshot={operationSnapshot}
          onDeviceSnapshot={setDeviceSnapshot}
        />
      </PageContainer>
      <ModalLayer isVisible={partitionFailure !== null} title="分区刷写失败">
        <PartitionFailureDialog failure={partitionFailure} onResolved={dismissPartitionFailure} />
      </ModalLayer>
    </AppShell>
  );
};
