...
const renderFileManager = (snapshot: DeviceSnapshotPayload | null = adbSnapshot) => {
  flushSync(() => {
    root.render(<FileManagerPage deviceSnapshot={snapshot} />);
  });
};
...
describe('FileManagerPage', () => {
  ...
  test('上传完成后重新读取当前设备目录', async () => {
    ...
    const cancelButton = host.querySelector('.nw-test-file-cancel') as HTMLButtonElement;
    expect(cancelButton.disabled).toBe(true);
    cancelButton.click();
    expect((invoke as unknown as ReturnType<typeof vi.fn>).mock.calls
      .filter(([name]) => name === 'operation_cancel')).toHaveLength(0);
  });

  test('上传进行中显示停止入口，重复点击只请求一次取消并在取消终态解锁', async () => {
    const upload = deferred<void>();
    const cancel = deferred<void>();
    (open as unknown as ReturnType<typeof vi.fn>).mockResolvedValue('C:\\chosen\\update.zip');
    (invoke as unknown as ReturnType<typeof vi.fn>).mockImplementation((name: string) => {
      if (name === 'files_upload') return upload.promise;
      if (name === 'operation_cancel') return cancel.promise;
    ...
    (host.querySelector('.nw-test-file-upload') as HTMLButtonElement).click();
    await waitUntil(() => {
      const button = host.querySelector('.nw-test-file-cancel') as HTMLButtonElement | null;
    ...
    cancelButton.click();
    cancelButton.click();
    await waitUntil(() => (invoke as unknown as ReturnType<typeof vi.fn>).mock.calls
      .filter(([name]) => name === 'operation_cancel').length === 1);
    expect(cancelButton.disabled).toBe(true);
    expect(cancelButton.textContent).toContain('正在停止');

    cancel.resolve();
    upload.reject(new Error('用户取消: 运行被用户取消'));
    await waitUntil(() => !(host.querySelector('.nw-test-file-upload') as HTMLButtonElement).disabled);
    expect((host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled).toBe(true);
  });

  test('下载失败后停止入口收起并显示安全错误文案', async () => {
    const download = deferred<void>();
    (save as unknown as ReturnType<typeof vi.fn>).mockResolvedValue('C:\\chosen\\update.zip');
    (invoke as unknown as ReturnType<typeof vi.fn>).mockImplementation((name: string) => {
      if (name === 'files_list') {
        return Promise.resolve([{ name: 'update.zip', full_path: '/sdcard/update.zip', is_directory: false, size_bytes: 512 }]);
      }
      if (name === 'files_download') return download.promise;
      return Promise.resolve(undefined);
    });
    renderFileManager();

    (host.querySelector('.nw-test-file-refresh') as HTMLButtonElement).click();
    await waitUntil(() => host.querySelector('.nw-test-file-download') !== null);
    (host.querySelector('.nw-test-file-download') as HTMLButtonElement).click();
    await waitUntil(() => !(host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled);

    download.reject(new Error('设备拒绝传输'));
    await waitUntil(() => host.textContent?.includes('设备拒绝传输') ?? false);
    expect((host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled).toBe(true);
    expect((host.querySelector('.nw-test-file-download') as HTMLButtonElement).disabled).toBe(false);
  });

  test('设备断开后不向已结束或失效的文件操作发送取消请求', async () => {
    const upload = deferred<void>();
    ...
    cancelButton.click();
    await flushPromises();
    expect(command.mock.calls.filter(([name]) => name === 'operation_cancel')).toHaveLength(0);

    upload.reject(new Error('设备已断开'));
    await waitUntil(() => host.textContent?.includes('设备已断开') ?? false);
  });

  test('取消请求失败时停止入口恢复可用，原操作仍保持忙态直到终态', async () => {
    const upload = deferred<void>();
    const command = invoke as unknown as ReturnType<typeof vi.fn>;
    (open as unknown as ReturnType<typeof vi.fn>).mockResolvedValue('C:\\chosen\\update.zip');
    command.mockImplementation((name: string) => {
      if (name === 'files_upload') return upload.promise;
      if (name === 'operation_cancel') return Promise.reject(new Error('取消请求失败'));
      return Promise.resolve([]);
    });
    renderFileManager();
    ...
    (host.querySelector('.nw-test-file-upload') as HTMLButtonElement).click();
    await waitUntil(() => !(host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled);
    (host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).click();

    await waitUntil(() => host.textContent?.includes('取消请求失败') ?? false);
    expect((host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled).toBe(false);
    expect((host.querySelector('.nw-test-file-upload') as HTMLButtonElement).disabled).toBe(true);
    expect(command.mock.calls.filter(([name]) => name === 'operation_cancel')).toHaveLength(1);

    upload.reject(new Error('用户取消: 运行被用户取消'));
    await waitUntil(() => !(host.querySelector('.nw-test-file-upload') as HTMLButtonElement).disabled);
  });

  test('安装 APK 进行中显示停止入口并在完成后解锁', async () => {
    const install = deferred<void>();
    const command = invoke as unknown as ReturnType<typeof vi.fn>;
    (open as unknown as ReturnType<typeof vi.fn>).mockResolvedValue('C:\\chosen\\manager.apk');
    command.mockImplementation((name: string) => {
      if (name === 'files_install_apk') return install.promise;
      return Promise.resolve(undefined);
    });
    renderFileManager();
    ...
    await waitUntil(() => !(host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled);
    expect((host.querySelector('.nw-test-file-install-apk') as HTMLButtonElement).disabled).toBe(true);

    install.resolve();
    await waitUntil(() => !(host.querySelector('.nw-test-file-install-apk') as HTMLButtonElement).disabled);
    expect((host.querySelector('.nw-test-file-cancel') as HTMLButtonElement).disabled).toBe(true);
  });
  ...
  test('使用 WPF 的设备文件工作台结构呈现空目录状态', () => {
    ...
    expect(host.querySelector('.nw-file-manager-log')).toBeNull();
