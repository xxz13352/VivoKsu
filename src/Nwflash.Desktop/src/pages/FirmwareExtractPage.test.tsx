...
describe('FirmwareExtractPage', () => {
  ...
  test('点击确认刷入后弹窗立即关闭，刷写仍在进行', async () => {
    const dialog = open as unknown as ReturnType<typeof vi.fn>;
    const command = invoke as unknown as ReturnType<typeof vi.fn>;
    dialog.mockResolvedValueOnce('C:\\private\\firmware\\vivo_ota.gz');
    ...
    command
      ...
      .mockResolvedValueOnce({ artifactId: 'firmware-1', name: 'boot.img', sizeBytes: 4 })
      .mockResolvedValueOnce({ partition: 'boot', taskCount: 1 })
      .mockImplementationOnce(() => new Promise<void>((resolve) => { resolveExecution = resolve; }));
    ...
    await waitUntil(() => host.querySelector('.nw-test-firmware-confirm-flash') !== null);
    (host.querySelector('.nw-test-firmware-confirm-flash') as HTMLButtonElement).click();
    await waitUntil(() => resolveExecution !== undefined);

    // 确认后弹窗必须立即消失：执行期间弹窗停留会遮住进度面板与「停止操作」，
    // 且按钮全禁、关闭按钮不渲染，永远点不掉（同线刷确认弹窗缺陷家族）。
    expect(host.querySelector('[role="dialog"]')).toBeNull();
    resolveExecution?.();
    await flushPromises();
  });

  test('刷写失败后恢复确认弹窗，可直接重试或取消，不会卡死', async () => {
    const dialog = open as unknown as ReturnType<typeof vi.fn>;
    const command = invoke as unknown as ReturnType<typeof vi.fn>;
    dialog.mockResolvedValueOnce('C:\\private\\firmware\\vivo_ota.gz');
    command
    ...
    await waitUntil(() => host.querySelector('.nw-test-firmware-confirm-flash') !== null);
    (host.querySelector('.nw-test-firmware-confirm-flash') as HTMLButtonElement).click();
    await waitUntil(() => host.textContent?.includes('镜像刷写失败') ?? false);

    // 工件仍在后端 staging：恢复弹窗允许直接重试，取消路径也保持可走。
    expect(host.querySelector('[role="dialog"]')).not.toBeNull();
    const cancelButton = Array.from(host.querySelectorAll<HTMLButtonElement>('[role="dialog"] button'))
