...
export const FirmwareExtractPage: FC = () => {
  ...
  const executePreparedArtifact = async () => {
    ...
    if (!pending || isWorking) {
      return;
    }
    setErrorText('');
    // 先关弹窗再执行（对齐快速刷写/线刷页模式）：执行期间弹窗停留会全屏
    // 遮住进度面板与「停止操作」，且按钮全禁、关闭按钮不渲染，永远点不掉。
    setPendingArtifact(null);
    setIsWorking(true);
    try {
      ...
      // 工件在后端 staging 中保留（capability 未失效即可重试）：恢复确认
      // 弹窗允许直接重试或取消，不必重新提取。
  ...
  return (
    <section className="nw-firmware-extract-page" aria-label="固件提取">
      ...
      {pendingArtifact ? (
        <ModalLayer isVisible title="确认刷入" onClose={() => setPendingArtifact(null)}>
          ...
          <div className="nw-driver-dialog-actions">
            ...
            <button
              type="button"
              className="nw-test-firmware-confirm-flash nw-dialog-confirm"
