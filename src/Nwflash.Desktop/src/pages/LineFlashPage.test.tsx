...
describe('LineFlashPage', () => {
  ...
  test('重新读取失败时保留上一次成功读取的分区表且不在页面内联报错', async () => {
    const command = invoke as unknown as ReturnType<typeof vi.fn>;
    command
    ...
    (host.querySelector('.nw-test-line-partitions-refresh') as HTMLButtonElement).click();
    // 失败详情由后端写入操作日志区展示；页面只等待读取结束，不再内联显示错误。
    await waitUntil(
      () => !(host.querySelector('.nw-test-line-partitions-refresh') as HTMLButtonElement).disabled,
    );
