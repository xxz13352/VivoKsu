...
export const RootPage: FC = () => {
  ...
  const extractFromServer = async () => {
    ...
    try {
      ...
      setErrorText(errorMessage(error, '服务器固件提取失败'));
      setSourceMode('local');
    } finally {
      setRootBusy(false);
    }
    ...
    <section className="nw-root-page" aria-label="Vivo ROOT">
      ...
      <section className="nw-root-workbench">
        <article className="nw-root-ota-source">
          <div>
            ...
            <button type="button" className="nw-test-root-ota-check" disabled={isBusy} onClick={() => void runOtaCheck()}>检测服务器固件</button>
            <small>{sourceLabel ? sourceLabel : (otaAvailable ? `已发现服务器固件：${otaLabel ?? ''}` : '从服务器获取时按需云提取 init_boot/boot/vendor_boot 分区')}</small>
          </div>
        </article>
      ...
      >
        ...
        <div className="nw-driver-dialog-actions">
          ...
          <button
            type="button"
            className="nw-test-root-confirm nw-dialog-confirm"
