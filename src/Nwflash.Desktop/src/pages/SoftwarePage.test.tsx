...
describe('SoftwarePage', () => {
  ...
  test('成功加载版本信息且不再拉取操作日志', async () => {
    ...
    expect(host.querySelector('.nw-software-logs')).toBeNull();
  });
  ...
  test('非法返回结构回退未知版本', async () => {
    (invoke as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce('bad version check');
    (invoke as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce('bad software status');
  ...
  test('版本诊断失败时仍展示独立的软件组件状态', async () => {
